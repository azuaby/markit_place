...
var unfantid;cartnoii=true;pushnoii=true; 
...
/*	$('#loginform').submit(function(){
	...
	*/
	...
	/* add tags */
	$(document).on('click',".button-medium",function(){
	...
	/* add meterails */
	$(document).on('click',".button-mediumer",function(){
	...
	$(document).on('click',".tag-rem",function(){
		$(this).remove();
   ...
   /* $('select#languages').selectList({ sort: true,
	...
	$("#itemform").submit(function(){
		...
		{
			$("#alert").show();
			$('#alert').text('Please select method for choosing color');
		...
		if(color_method=='manual')
		{
			...
			{
				$("#alert").show();
				$('#alert').text('Please select item colors');
		...
		if(processing_time == ''){
			$("#alert").show();
			$('#alert').text('Please Select Processing time');
	...
	function dispute(oid){
		var BaseURL=getBaseURL();
		window.location = BaseURL+'userdispute/'+oid;
	}
	...
	$(document).on('click',".shop-favorite",function(){
		var loguserid = $("#loguserid").val();
		if(loguserid == 0){
			alert("Please Login");
			return false;
	...
	if(logid == 0){
		...
		//$("#flw_"+usrid).text('waiting ...');
			...
			$.post(baseurl+'addflw_usrs', {"usrid":usrid},
				...
				var cmtval = '<span class="follow" id="unfoll'+usrid+'"><button type="button" id="unfollow_btn'+usrid+'" class="greebtn" onclick="deletefollows('+usrid+')"><span class="unfoll'+usrid+'" >Following</span></button></span>';
	...
	if(logid == 0){
		...
		//$("#flw_"+usrid).text('waiting ...');
			...
			$.post(baseurl+'delerteflw_usrs', {"usrid":usrid},
				...
				$("#foll"+usrid).hide(); $("#mainfoll"+usrid).hide();
				$("#unfoll"+usrid).hide(); $("#mainunfoll"+usrid).hide();
				var cmtval = '<span class="follow" id="foll'+usrid+'"><button type="button" id="follow_btn'+usrid+'" class="btnblu" onclick="getfollows('+usrid+')"><span class="foll'+usrid+'" >Follow</span></button></span>';
				$("#changebtn"+usrid).html(cmtval); $("#mainchangebtn"+usrid).html(cmtval);
...
function addfollows(usrid){
	...
	if(logid == 0){
		...
		return false;
			...
			$.ajax({
				type: "POST",
				url: baseurl+'addflw_usrs',
				data: {"usrid":usrid},
	
				beforeSend: function() {
				$(".follow_btn"+usrid).attr("disabled", "disabled");
				},

				success: function(datas) {
				$("#foll"+usrid).hide(); 
				$("#unfoll"+usrid).hide(); 
				var cmtval = '<span class="follow-wiz" id="unfoll'+usrid+'"><button type="button" class="unfollow_btn'+usrid+'" id="verif_following" onclick="delfollows('+usrid+')"><span class="unfoll'+usrid+'" >Following</span></button></span>';
...
function delfollows(usrid){
	...
	if(logid == 0){
		...
		return false;
				...
				$.ajax({
				type: "POST",
				url: baseurl+'delerteflw_usrs',
				data: {"usrid":usrid},
				
				beforeSend: function() {
				$(".unfollow_btn"+usrid).attr("disabled", "disabled");
				},

				success: function(datas) {
				$("#foll"+usrid).hide(); 
				$("#unfoll"+usrid).hide(); 
				var cmtval = '<span class="follow-wiz" id="foll'+usrid+'"><button type="button" class="follow_btn'+usrid+'" id="verif_follow" onclick="addfollows('+usrid+')"><span class="foll'+usrid+'" >Follow</span></button></span>';
				$("#changebtn"+usrid).html(cmtval);	
				return false;
				}
		});
	...
	if(logid == 0){
		...
		//$("#flw_"+usrid).text('waiting ...');
			...
			$.post(baseurl+'addflw_usrs', {"usrid":usrid},
				...
				$("#foll"+usrid).hide();
				$("#unfoll"+usrid).hide();
				var cmtval = '<span class="follow" id="unfoll'+usrid+'"><div class="profilImgName"><button type="button" id="unfollow_btn'+usrid+'" class="greebtn" onclick="deletefollowsuserpro('+usrid+')" style="height: 57px;margin-top:26px;border-radius:0px;box-shadow:none;"><span class="unfoll'+usrid+'" >Following</span></button></div></span>';
				$("#changebtn"+usrid).html(cmtval);
				//$(".foll"+usrid).text('Following... ');
	...
	if(logid == 0){
		...
		//$("#flw_"+usrid).text('waiting ...');
			...
			$.post(baseurl+'delerteflw_usrs', {"usrid":usrid},
				...
				$("#foll"+usrid).hide();
				$("#unfoll"+usrid).hide();
				var cmtval = '<span class="follow" id="foll'+usrid+'"><div class="profilImgName"><button type="button" id="follow_btn'+usrid+'" class="btnblu" onclick="getfollowsuserpro('+usrid+')" style="height: 57px;margin-top:26px;border-radius:0px;box-shadow:none;"><span class="foll'+usrid+'" >Follow</span></button></div></span>';
				$("#changebtn"+usrid).html(cmtval);
...
function removeusrimgicon(val){
	...
	$('#show_url_icon').attr({src: baseurl+'media/avatars/thumb350/usrimg.jpg'});
	$('#removeimgicon').hide();
}
...
function removecart (itemId, userId, shopId, id) {
	...
	$.ajax({
      url: $baseurl+"paypals/delete_cart_item/"+itemId+"/"+userId+"/"+shopId,
      type: "post",
...
function removegiftcart (giftcartid) {
	...
	$.ajax({
      url: $baseurl+"paypals/delete_giftcart_item?giftcartid="+giftcartid,
      type: "GET",
      dataType: "html",
      success: function(responce){
      	//$('#gfremoved').show();
  	    ...
  	    $('.totalItemsCart').html(responce+" item(s)");
  	    $('.totalYouHave').html("You have "+responce+" Items in your Cart");
      } 
    });
	}else{
		return false;
...
function selectChange(itemId, userId, shopId, id) {
	...
	$.ajax({
	      url: $baseurl+"paypals/delete_cart_item/"+itemId+"/"+userId+"/"+shopId+"/"+qnty+"/"+couponnId,
	      type: "post",
	      dataType: "json",
	      data : { 'currentship': cartshipping},
	      beforeSend: function () {
...
function checkout (itemids,merchid,shipamt) {
	...
	var tax_id_val = $("#tax_tot_val").val();
	if(tax_id_val!='')
		{
		tax_id_val = tax_id_val;
		}
	else
		{
		tax_id_val = 0;
	...
	$.ajax({
	      ...
	      type: "post",
	      data : { 'item_id': itemids,'shippingid': shippingid,'shipamt': shipamt,'taxamt':tax_id_val,'couponId': couponnId,'userEnterCreditAmt': userEnterCreditAmt,'giftCardId': giftCardId},
	      dataType: "html",
			beforeSend: function() {
				$(adaptiveload).show();
	      ...
	      success: function(responce){
	    	  resp = responce.indexOf("item error");
	    	  if(resp>0)
	    	  {
	    		  $("#itemerror").show();
	    		  $(adaptiveload).hide();
	...
	if(logid == 0){
		...
		if(commentss!='' && comment_status ){
			...
			$.ajax({
				...
				data: {"userid":userid,"itemid":itemid, "commentss":commentsss, "comment":comment},
				beforeSend: function() {
					$('.post-loading').show();
				...
				success: function(datas) {
					...
					datas = $.trim(datas);
					var appval = '<li class="comment  delecmt_'+datas+'" commid="'+datas+'" cuid="'+userid+'" ><a class="milestone" ></a><span class="vcard"><a href="'+baseurl+'people/'+usernames+'" class="url"><img src="'+baseurl+'media/avatars/thumb70/'+userimges+'" alt="" class="photo" style="'+radius+'"></a></span><span class="vcard cmntusername" style="position:relative;"><a href="'+baseurl+'people/'+usernames+'" class="url"><span class="fn nickname">'+usernames+'</span></a></span><p class="c-text" id="txt1'+datas+'"  style="display:none;"><textarea id="txt1val'+datas+'" maxlength="180" style="float: right; overflow: auto; resize: none; height: 50px; width: 541px; padding: 5px 0px 0px 10px;" onkeyup="ajaxuserautoc(this.value,\'txt1val'+datas+'\',\'comment-autocomplete'+datas+'\');">'+commentss+'</textarea><button class="btn-blue-post btn-savecmd" onclick = "return editcmntsave('+datas+')" >Save comment</button></p><div class="comment-autocomplete comment-autocomplete'+datas+'" style="display: none;left:43px;width:548px;"><ul class="usersearch"></ul></div><p id="oritext'+datas+'" style="margin-top:-10px;">'+commentsss+'</p><div id="oritextvalafedit'+datas+'"></div><p class="c-reply"></p><p class="c-reply"><button class="edit-comment" onclick = "return editcmnt('+datas+')" >Edit</button><span class="bar"></span><button class="delete-comment"  onclick = "return deletecmnt('+datas+')" >Delete</button></p></li>';
		...
		if(commentss!='' && comment_status ){
			...
			if(commentsss.match(pattern)){
				...
				for (var i = 0; i < result.length; i++) {
				    ...
				    var link = "<span class='hashatcolor'>@</span><a href='"+baseurl+"people/"+desired+"'>"+desired+"</a>";
				    var replacestr = '@'+desired;
				    //var replacestr = result[i];
				    commentsss = commentsss.replace(replacestr,link);
				}
			...
			if(hashComment.match(hashPattern)){
				...
				for (var i = 0; i < result.length; i++) {
				    ...
				    var link = "<span class='hashatcolor'>#</span><a href='"+baseurl+"hashtag/"+desired+"'>"+desired+"</a>";
				    var replacestr = '#'+desired;
			...
			$.ajax({
				...
				success: function(datas) {
					...
					var appval = '<li class="comment  delecmt_'+datas+'" commid="'+datas+'" cuid="'+userid+'" ><a class="milestone" ></a><span class="vcard"><a href="'+baseurl+'people/'+usernames+'" class="url"><img src="'+baseurl+'media/avatars/thumb70/'+userimges+'" alt="" class="photo" style="'+radius+'"></a></span><span class="vcard cmntusername" style="position:relative;"><a href="'+baseurl+'people/'+usernames+'" class="url"><span class="fn nickname">'+usernames+'</span></a></span><p class="c-text" id="txt1'+datas+'"  style="display:none;"><textarea id="txt1val'+datas+'" maxlength="180" style="float: right; overflow: auto; resize: none; height: 50px; width: 541px; padding: 5px 0px 0px 10px;" onkeyup="ajaxuserautoc(event,this.value,\'txt1val'+datas+'\',\'comment-autocomplete'+datas+'\',\''+datas+'\');">'+commentss+'</textarea><button class="btn-blue-post btn-savecmd" onclick = "return editcmntsave('+datas+')" >Save comment</button></p><div class="comment-autocomplete comment-autocomplete'+datas+'" style="display: none;left:43px;width:548px;"><ul class="usersearch"></ul></div><p id="oritext'+datas+'" style="margin-top:-10px;">'+commentsss+'</p><div id="oritextvalafedit'+datas+'"></div><p class="c-reply"></p><p class="c-reply"><button class="edit-comment" onclick = "return editcmnt('+datas+')" >Edit</button><span class="bar"></span><button class="delete-comment"  onclick = "return deletecmnt('+datas+')" >Delete</button></p></li>';
	...
	$('#webupload').click(function(){
		...
		$('#getimg').click(function(){
			...
			if(text_url != '' && url_status){
				url_status=false;
				text_urlval = text_url.indexOf("https://");
				if(text_urlval<0)
					text_url = text_url.replace("http://", "");
				else
					text_url = text_url.replace("https://", "");
				          ...
				          $('#images_orig').attr('src',obj[0]);
							...
							$('#givenlink').val(text_url);
					  		...
					  		$('#saveimgs').click(function(){
					      		...
					      		image_nameval = image_name.indexOf("https://");
					      		if(image_nameval<0)
					      			image_name = image_name.replace("http://", "http");
					      		else
					      			image_name = image_name.replace("https://", "https");
					      		if(additem_title == ''){
...
function itemcou(id){	
	...
	var BaseURL=getBaseURL();
	//alert(id);
	var imgselector = '#img_id'+id+' img:first';
	var likedbtncnt = $("#likedbtncnt").val();
	var likebtncnt = $("#likebtncnt").val();
	//alert(likedbtncnt);
		var loguserid = $("#loguserid").val();
	...
	var BaseURL=getBaseURL();
		...
		$('#videourrls').hide();
		$('#showprofile').hide();
		//alert($(imgselector).attr('src'));
           $("#thum_img").attr("src",$(imgselector).attr('src'));
	   ...
	   var title_url = $('#figcaption_title_url'+id).html();
		   ...
		   $("#fav_countsvv").text(fav_counts);
			...
			var urlss = BaseURL+'listing/'+id +'/'+ title_url;
			//alert(urlss);
	...
	function share_user_bio(uname,id){
		  ...
		  //if($("#usecreditamount"+shopid).attr("checked")){
			 ...
			 // if(creditamnts == 0){  
			  ...
			  $('#usercreditamntchek').click(function(){
				 ...
				 var tax = parseFloat($("#tax_tot_val").val());
					if(tax=='' || isNaN(tax))
						{
						tax = 0;
						}
				 ...
				 if(totalval >= userentercreditamt){
					 ...
					 var grandTotal = (credit_reduced+totalshipp+tax).toFixed(2);
	  ...
	  function alertusecreditamount(shopid,userid){
		  alert("Your credit amount is null");
	  }
...
function getLatLong(kilometer){
  ...
  getScript("http://maps.gstatic.com/intl/en_us/mapfiles/api-3/4/4a/main.js");
  })(); 
...
function show_fashion_image(imgname,id)
{
		$('#popup_container').show();
		$('#popup_container').css({"opacity":"1"});
		$('#videourrls').hide();
		...
		$('.contact-seller').hide();
		$('.write-review').hide();
		$("#upload_fashion_image").show();
		document.getElementById('fashion_image').src = imgname;
		$("#imgfashionid").val(id);
		//$("#imgfashioncount").val(count);

}


function show_prev_fashion_image(imgarray)
{
	var BaseURL=getBaseURL();
	idval = $("#imgfashionid").val();
	iteid = $("#itemid").val();
	imgcount = $("#imgfashioncount").val();
	testval = jQuery.inArray( idval, imgarray );
	...
	$.ajax({
		type: "POST",
		url: BaseURL+'view_prev_fashion_image',
		data: {'id':previd,'itemid':iteid,'imagearray':imgarray},
		success: function(data) {
			data = $.trim(data);
			data = data.split("***");
			imgname = BaseURL+"media/avatars/original/"+data[0];
			
			document.getElementById('fashion_image').src = imgname;
			$("#imgfashionid").val(data[1]);
		}
	});	
}

function show_next_fashion_image(imgarray)
{
	var BaseURL=getBaseURL();
	idval = $("#imgfashionid").val();
	iteid = $("#itemid").val();
	imgcount = $("#imgfashioncount").val();
	testval = jQuery.inArray( idval, imgarray );
	if (testval<0 || testval==imgcount-1) {
		nextid = 0;
	}
	else
	{
		nextid = parseInt(testval) + 1;
	}	
	$.ajax({
		type: "POST",
		url: BaseURL+'view_next_fashion_image',
		data: {'id':nextid,'itemid':iteid,'imagearray':imgarray},
		success: function(data) {
			data = $.trim(data);
			data = data.split("***");
			imgname = BaseURL+"media/avatars/original/"+data[0];
...
function review(sellerid,userid)
{
	...
	$.ajax({
		type: "POST",
		url: BaseURL+'rating',
		data: {"order_id":orderid,"user_id":userid,"seller_id":sellerid,"rate_val":rating_val,"review_cont":review_content,"loadcount":load_count},
		success: function(data) {
			$("#disp").html(data);
			$("#loadcount").val("2");
...
function view_review(sellerid,pageno)
{
	var BaseURL=getBaseURL();
	base_url = BaseURL+'view_review';
	$.ajax({
		type: "POST",
		url:base_url ,
		data: "sellerid="+sellerid+"&page_no="+pageno,
		success: function(data) {
			$("#res").html(data);
		}
	});	
...
$(document).on('keyup','#review', function() {
	var maxLen = 250; 
	//alert(maxLen);
	if ($('#review').val().length >= maxLen) {
	//var msg = "You have reached your maximum limit of characters allowed";
	//alert(msg);
		document.getElementById('review').value = $('#review').val().substring(0, maxLen);
	 		$('#err_txt').css('color', 'red');
		$('#err_txt').show();
	$('#err_txt').text('You have reached your maximum limit of characters allowed');
	 }
	else{ 
		//document.getElementById('text_num').value = maxLen - $('#review').val().length;
		$('#err_txt').hide();
	}
});
...
function verificate_cat(id){
	//alert(id);
	Baseurl = getBaseURL();
	$.ajax({
	      url: Baseurl+"category_verification/"+id,
	      type: "post",
	      dataType: "html",
	      beforeSend: function () {
	    	  $(".loaddsusId").show();
