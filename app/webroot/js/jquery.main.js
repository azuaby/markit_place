...
var Fancy = {
	...
	buttons: function() {
		// Follow links
		$('#content,#sidebar,#profile-content')
			.on({
				click : function(event) {
				...
				mouseover : function(){
					...
					if($this.hasClass('following') && this.lastChild && this.lastChild.nodeType == 3) {
						$this.data('following-string', this.lastChild.nodeValue);
						 $this.addClass('dimend');this.lastChild.nodeValue = gettext('Unfollow');
					}
				},
				mouseout : function(){
					var $this = $(this);
					if($this.hasClass('following') && this.lastChild && this.lastChild.nodeType == 3) {
                                            ...
                                            if ($this.hasClass('following'))
                                                $this.removeClass('dimend');this.lastChild.nodeValue = gettext('Following');
                                        }
				}
...
jQuery(function($){
	...
	$nav
		.on(
			...
			'li.fantaci, .menu-contain-gift li'
		)
...
(function($){
	...
	$links
		.on('focus', function(){
			this.parentNode.className = 'fantaci hover';
		})
		.on('blur', function(){
			this.parentNode.className = 'fantaci';
		})
		.on('touchstart', function(){
			if(this.parentNode.className == 'fantaci hover'){
				return true;
			}else{
				this.focus();
				return false;
			...
			.find('>.step.step2')
				...
				.find('.btn-blue-embo-fetch')
					.click(function(){
						...
						url = $step.find('input.url_').val().trim().replace(/^http:\/\//i,'');
...
jQuery(function($){
	var $fancy_share = $('#fancy-share'), dlg_share = $.dialog('fancy-share');
	...
	function add_items(json){
		var list = json.list, $more = $list.find('>li.load-more');
		...
		if(json.next) {
			...
			$more.data('cursor', json.next).appendTo($list);
		} else {
			$more.remove();
		}
	};
	...
	$inp
		.on('changed', function(){
			val = $.trim($inp.val());
			if(!val.length || val.indexOf('@') >= 0) return $list.hide();
			$list.hide().html('');
	...
	$fancy_share
		.on('reset', function(){
			var $this = $(this);
			$this.removeClass('list-share thing-share gift-share comment-share').data({url:'',txt:'',img:''});
		})
		.on('open_comment', function(event, btn){
			...
			$.ajax({
				...
				success  : function(json){
					...
					$this
						.find('#direct-link-input').val(json.short_url).end()
		...
		.on('open_thing', function(event, btn){			
			var $this = $(this).trigger('reset').addClass('thing-share'), $btn=$(btn), tid=$btn.attr('tid'), uname=$this.attr('uname'), url, tname, img, iframe_h;

			img = $btn.parent().prev('img').get(0);
			if (!img) img = $btn.parents(".figure-item").find(".figure > img").get(0);
			if (!img) img = $('.fig-image > img').get(0);
			iframe_h = Math.ceil(454/img.width * img.height) + 105;

			var thing_path = $btn.closest('a').attr('href');
			if(!thing_path){
				thing_path = $btn.parents("div.figure-item").find("a.figure-img").attr('href');
			}
			$this
				.data({
					url : url='http://'+location.host+thing_path+'?ref='+uname,
				...
				.find('.fig-info')
					...
					.find('#share-embed-input').val('<iframe src="http://'+location.host+'/embed/'+tid+'?ref='+uname+'" width="480" height="'+iframe_h+'" allowtransparency="true" frameborder="0" style="width:480px;height:'+iframe_h+'px;margin:0 auto;border:0"></iframe>').end()
				.end();

			if($this.attr('amex') == 'true') $this.data('amex', tname+' '+url+' via @fancy + @AmericanExpress #AmexFancy');
			...
			$.ajax({
				...
				success  : function(json){
					...
					if ($this.attr('amex') == 'true') {
						var $tw = $this.find('.share-via a.tw'), href  = $tw.attr('href');
						$tw.attr('href', href.replace(/http:\/\/[^ ]+ via/, enc_short_url+' via'));
					}
				}
			});
		...
		.on('open_list', function(event, btn){
			var $this=$(this).trigger('reset').addClass('comment-share list-share'), $btn=$(btn), id=$btn.attr('lid'), oid=$btn.attr('loid'), url, $imgs, images=[];
		...
		.on('open_gift', function(event, btn){
			var $this = $(this).trigger('reset').addClass('comment-share gift-share'), $btn=$(btn), url;
			...
			$this
				...
				.find('#direct-link-input').val(url);
		...
		.on('click', '.btn-share', function(){
			...
			$this.disable();
                        ...
                        var otype = $fancy_share.attr('otype');
...
jQuery(function($){
	...
	$('#content,#sidebar').delegate('.btn-clone-list', 'click', function(event){
