...
/*	$('#loginform').submit(function(){
	...
	*/
	...
	$("#countrys").change(function(){
		var cate_id = $("#countrys :selected").val();
		// alert(cate_id);
		var items="";

		if(cate_id != ''){
			 $.getJSON(BaseURL+"suprcountry?cate_id="+cate_id+"&suprsub=yes",function(data){
			    console.log(data);
				...
				//$("#state-container").removeClass('inactive');
				//$("#state-container label").removeClass('invisible');
				$("#state-container").show();
				$("#state-selectbx").html(items); 
				$("#state-container .out").html('Select state');
				$("#area-container .out").html('Select area');
			});
			
			$.getJSON(BaseURL+"country_phonecode?cate_id="+cate_id,function(data){
                $("#mcode").val(data);
			});
			
		}else{
			$("#state-container").addClass('inactive');
			$("#state-container label").addClass('invisible');
			$("#state-selectbx").html(''); 
		}		
	});
	
	$("#state-selectbx").change(function(){
		var cate_id = $("#state-selectbx :selected").val();
		// alert(cate_id);
		var items="";
		if(cate_id != ''){
			 $.getJSON(BaseURL+"suprcountry?cate_id="+cate_id+"&suprsub=no",function(data){
				// alert(data);
				// return false;
				items+="<option value=''>Select Category</option>";
				$.each(data,function(index,cate) 
				...
				//$("#area-container").removeClass('inactive');
				//$("#area-container label").removeClass('invisible');
				$("#area-container").show();
				$("#area-selectbx").html(items); 
				$("#area-container .out").html('Select category');
			});
		}else{
			$("#area-container").addClass('inactive');
			$("#area-container label").addClass('invisible');
			$("#area-selectbx").html(''); 
		}		
	});

	$("#shipping_countrys").change(function(){
		var cate_id = $("#shipping_countrys :selected").val();
		// alert(cate_id);
		var items="";

		if(cate_id != ''){
			 $.getJSON(BaseURL+"shpng_country?cate_id="+cate_id+"&suprsub=yes",function(data){
				...
				//$("#state-container").removeClass('inactive');
				//$("#state-container label").removeClass('invisible');
				$("#shpng_state-container").show();
				$("#shpng_state-selectbx").html(items); 
				$("#shpng_state-container .out").html('Select state');
				$("#shpng_area-container .out").html('Select area');
			});
		...
		///	});
			...
			$("#shpng_state-container").addClass('inactive');
			$("#shpng_state-container label").addClass('invisible');
			$("#shpng_state-selectbx").html(''); 
		}		
	});
	
	$("#shpng_state-selectbx").change(function(){
		var cate_id = $("#shpng_state-selectbx :selected").val();
		// alert(cate_id);
		var items="";
		if(cate_id != ''){
			 $.getJSON(BaseURL+"shpng_country?cate_id="+cate_id+"&suprsub=no",function(data){
				// alert(data);
				// return false;
				items+="<option value=''>Select Category</option>";
				$.each(data,function(index,cate) 
				...
				//$("#area-container").removeClass('inactive');
				//$("#area-container label").removeClass('invisible');
				$("#shpng_area-container").show();
				$("#shpng_area-selectbx").html(items); 
				$("#shpng_area-container .out").html('Select category');
			});
		}else{
			$("#shpng_area-container").addClass('inactive');
			$("#shpng_area-container label").addClass('invisible');
			$("#shpng_area-selectbx").html(''); 
		}		
	});
   ...
   /* $('select#languages').selectList({ sort: true,
	...
	$("#itemform").submit(function(){
		...
		{
			$("#alert").show();
			$('#alert').text('Please select delivery type');
...
function signform(){
	...
	if(firstname == ''){
		$("#alertFName").show();
		$("#badmessage").hide();
		$('#alertFName').text('Name is required');
			$('#firstname').focus()
	...
	if(lastname == ''){
		$("#alertLName").show();
		$("#badmessage").hide();
		$('#alertLName').text('Name is required');
			$('#lastname').focus()
	...
	if(countrys == ''){
		$("#alert_Cntry").show();
		$("#badmessage").hide();
		$('#alert_Cntry').text('Country is required');
			$('#countrys').focus()
	...
	if(!(rpassword == password)){
		$("#alertrPass").show();
		$("#badmessage").hide();
		$('#alertrPass').text('Password does not Match');
			$('#rpassword').focus()
		$('#rpassword').keydown(function() {
			$('#alertrPass').text('Enter Valid Password');
		});
...
function set_deltypes(){
	var id = $('#cate_id').attr('value');
    var dtype = window.cat_type[id];
    var type = $('#deltype').val(dtype);
}
