...
/*	$('#loginform').submit(function(){
	...
	*/
	...
	$("#cate_id").change(function(){
		...
		var items="";
		if(cate_id != ''){
			 $.getJSON(BaseURL+"mobile/suprsubcategry?cate_id="+cate_id+"&suprsub=yes",function(data){
	...
	$("#categ-selectbx-2").change(function(){
		...
		var items="";
		if(cate_id != ''){
			 $.getJSON(BaseURL+"mobile/suprsubcategry?cate_id="+cate_id+"&suprsub=no",function(data){
				// alert(data);
				// return false;
				items+="<option value=''>Select Category</option>";
				$.each(data,function(index,cate) 
	...
	*/
	...
	/* add meterails */
	$(".button-mediumer").click(function(){
   ...
   /* $('select#languages').selectList({ sort: true,
		...
		var htms = '<tr class="new-shipping-location '+lctn+'">';
			...
			htms += '<td>';
				htms += '<div class="input-group input-group-price price-input primary-shipping-price">';
					...
					htms += '<div class="ui-input-text ui-body-inherit ui-corner-all ui-shadow-inset"><input type="text" value="" class="money text text-small" name="country_shipping['+lctn+']['+incrmt_val+'][primary]"></div>';
				htms += '</div>';
			htms += '</td>';  
	...
	$(".input-group-close a").click(function(){
		//alert(this.id);
		$("."+this.id).remove();
		return false;
	});
	...
	$("#add-favorite").click(function(){
		...
		if(loguserid == 0){
			...
			$.post(BaseURL+'mobile/addfavitems', {"itemid":listingid},
				function(datas) {
					if(datas != 0){
	...
	$(".shop-favorite").click(function(){
		...
		if(loguserid == 0){
			...
			$.post(BaseURL+'mobile/addfavshops', {"shopid":shopid},
				function(datas) {
					if(datas != 0){
	...
	$('#btn-doneid').click(function(){
		...
		$.post(BaseURL+'mobile/totaladduserlist', {"alldata":data,"itemid":id},
					function(datas) {
					$('#popup_container').hide();
					$('#popup_container').css({"opacity":"0"});
					window.history.go(-1);
	...
	$('#list_createid').click(function(){
		...
		if(newlist_val!=''){
		$.post(BaseURL+'mobile/adduserlist', {"itemid":id,"newlist":newlist_val},
				function(datas) {
				if(datas!='' ){
					var appval = '<li style="list-style:none;margin:7px;"><input type="checkbox" value="'+datas+'" name="category_items[]" checked="checked" /><span style="margin-left:3px;">'+datas+'</span></br></li>';
	...
	if(logid == 0){
		...
		//$("#flw_"+usrid).text('waiting ...');
			...
			$.post(baseurl+'addflw_usrs', {"usrid":usrid},
				...
				var cmtval = '<span class="follow" id="unfoll'+usrid+'"><span id="unfollid"><button class="greenbtn" style="background:#339EF0;color:#FFFFFF;text-shadow:none;left:-10px;width:100px;height:35px;top:-5px;" id="unfollow_btn'+usrid+'" onclick="deletefollows('+usrid+')"><span class="unfoll'+usrid+'" style="font-size:13px;position:relative;top:-5px;">Following</span></span></button></span>';
				$("#changebtn"+usrid).html(cmtval).trigger("create"); $("#mainchangebtn"+usrid).html(cmtval).trigger("create");
	...
	if(logid == 0){
		...
		//$("#flw_"+usrid).text('waiting ...');
			...
			$.post(baseurl+'delerteflw_usrs', {"usrid":usrid},
				...
				$("#foll"+usrid).hide(); $("#mainfoll"+usrid).hide();
				$("#unfoll"+usrid).hide(); $("#mainunfoll"+usrid).hide();
				var cmtval = '<span class="follow" id="foll'+usrid+'"><span id="follid"><button style="left:-10px;width:100px;height:35px;top:-5px;" class="btnblu" id="follow_btn'+usrid+'" onclick="getfollows('+usrid+')"><span style="font-size:13px;position:relative;top:-5px;" class="foll'+usrid+'" >Follow</span></span></button></span>';
				$("#changebtn"+usrid).html(cmtval).trigger("create"); $("#mainchangebtn"+usrid).html(cmtval).trigger("create");
	...
	if(logid == 0){
		...
		//$("#flw_"+usrid).text('waiting ...');
			...
			$.post(baseurl+'addflw_usrs', {"usrid":usrid},
				...
				$("#foll"+usrid).hide();
				$("#unfoll"+usrid).hide();
				var cmtval = '<span class="follow" id="unfoll'+usrid+'"><center><div class="profilImgName"><button style="background:#339EF0;color:#FFFFFF;text-shadow:none;width:100px;height:35px;margin-top:12px;" type="button" id="unfollow_btn'+usrid+'" class="editPrfcls" onclick="deletefollowsuserpro('+usrid+')" style="height: 57px;"><span class="unfoll'+usrid+'" style="font-size:13px;position:relative;top:-5px;">Following</span></button></div></center></span>';
				$("#changebtn"+usrid).html(cmtval).trigger("create");
				//$(".foll"+usrid).text('Following... ');
	...
	if(logid == 0){
		...
		//$("#flw_"+usrid).text('waiting ...');
			...
			$.post(baseurl+'delerteflw_usrs', {"usrid":usrid},
				...
				$("#foll"+usrid).hide();
				$("#unfoll"+usrid).hide();
				var cmtval = '<span class="follow" id="foll'+usrid+'"><center><div class="profilImgName"><button style="width:100px;height:35px;margin-top:12px;" type="button" id="follow_btn'+usrid+'" class="editPrfcls" onclick="getfollowsuserpro('+usrid+')" style="height: 57px;"><span class="foll'+usrid+'" style="font-size:13px;position:relative;top:-5px;">Follow</span></button></div></center></span>';
				$("#changebtn"+usrid).html(cmtval).trigger("create");
...
function removecart (itemId, userId, shopId, id) {
	...
	$.ajax({
      ...
      success: function(responce){
    	  ...
    	  if (obj[0] != null) {
    	 $(eleid).html(obj[0]).trigger("create"); 
    	 }else {
    		 window.location.assign(obj['url']+'mobile/cart');
    	 }
    	 $('.totalItemsCart').html(obj[1]+" item(s)").trigger("create");
    	 $('.totalYouHave').html("You have "+obj[1]+" Items in your Cart").trigger("create");
      } 
    });
	}else{
		return false;
...
function getColor (color) {
	$baseurl = getBaseURL();
	$.ajax({
	      url: $baseurl+"mobile/color/"+color+"/1",
	      type: "post",
	      dataType: "html",
	      success: function(responce){
	          //alert(eleid);
...
function testajax(id)
{
	$baseurl = getBaseURL();
	$.ajax({
	      url: $baseurl+"mobilepaypals/delete_cart_item/",
	      type: "post",
	      
	      data : { 'id': id},
	   
	      success: function(responce){
	       $("#test").html(responce).trigger("create");
	      }
	});
}
...
function giftcardusebuynow (itemids,merchid,shipamt) {
	...
	$.ajax({
	      url: baseurl+"mobile/gfcrdcheckout/",
	      type: "post",
	      data : { 'item_id': itemids,'shippingid': shippingid,'shipamt': shipamt,'couponId': couponnId,'userEnterCreditAmt': userEnterCreditAmt,'giftCardId': giftCardId},
	      dataType: "html",
			beforeSend: function() {
	      ...
	      success: function(responce){
	    	  //alert(responce);
	    	  window.location.href=baseurl+"mobile/payment-successful";
	    	  //$('#paypalfom').html(responce);
	    	  //$('#paypal').submit();
	      }
	});
...
function deletecmnt(id){
		...
		//alert(eleid);
		if(id!=''){
		$.post(baseurl+'mobile/deletecomments', {"addid":id,"itemid":itemid},
			function(datas) {
			$(eleid).remove();
			return false;
		}
...
function deleteadditem(id){
	...
	if (x){
	if(id!=''){
	$.post(baseurl+'mobile/deleteadditem', {"addid":id},
		function(datas) {
			$(eleid).remove();
		//return false;
	}
...
function editcmntsave(id){
	...
	if(id!=''){
		...
		$.ajax({
			type: "POST",
			url: baseurl+'mobile/editcommentsave',
	...
	if(logid == 0){
		...
		if(commentss!=''){
			...
			for (var i = 0; i < result.length; i++) {
			    ...
			    //alert(result[i]);
			    var link = "<a href='"+baseurl+"mobile/people/"+result[i]+"'>"+result[i]+"</a>";
			    var replacestr = result[i];
			}
			...
			$.ajax({
				type: "POST",
				url: baseurl+'mobile/addcomments',
				...
				success: function(datas) {
					...
					var appval = '<span class="comment  delecmt_'+datas+'" commid="'+datas+'" cuid="'+userid+'" ><a class="milestone" ></a><table style="width:100%;"><thead><tr><th colspan="2"></th></tr></thead><tbody><tr><td style="width:45px;"><span class="vcard"><a data-ajax="false" href="'+baseurl+'mobile/people/'+usernames+'" class="url"><img src="'+baseurl+'media/avatars/thumb70/'+userimges+'" alt="" class="photo" style="width:40px;height:40px;'+radius+'"></a></span</td><td><span class="vcard cmntusername" style="position:relative;top:10px;"><a data-ajax="false" style="text-decoration:none;" href="'+baseurl+'mobile/people/'+usernames+'" class="url"><span class="fn nickname">'+usernames+'</span></a></span><p class="c-text" id="txt1'+datas+'"  style="display:none;"><br /><br /><textarea id="txt1val'+datas+'" maxlength="180" style="float: right; overflow: auto; resize: none; height: 50px; padding: 5px 0px 0px 10px;">'+commentss+'</textarea><button class="btn-blue-post btn-savecmd" style="margin-top:70px;background:#5690BB;color:#FFFFFF;text-shadow:none;" onclick = "return editcmntsave('+datas+')" >Save comment</button></p><p id="oritext'+datas+'" style="margin-top:10px;">'+commentsss+'</p><div id="oritextvalafedit'+datas+'"></div></td></tr></table><p class="c-reply"></p><p class="c-reply"><button style="background:#5690BB;text-shadow:none;color:#FFFFFF;" class="edit-comment" onclick = "return editcmnt('+datas+')" >Edit</button><span class="bar"></span><button style="background:#5690BB;text-shadow:none;color:#FFFFFF;" class="delete-comment"  onclick = "return deletecmnt('+datas+')" >Delete</button></p></span>';
					$("#few").append(appval).trigger("create");
	...
	$('#webupload').click(function(){
		...
		$('#getimg').click(function(){
			...
			if(text_url != '' && url_status){
				...
				var baseurl = getBaseURL()+'mobile/additemusingurl?url='+text_url;
				$.ajax({
				      url: baseurl,
				      type: "GET",
				      dataType: "html",
							...
							$('#givenlink').val(text_url);
					  		...
					  		$('#saveimgs').click(function(){
					      		...
					      		if(item_save){
					      			item_save=false;
						      		var baseurl = getBaseURL();
						      		var urls = baseurl +'mobile/additemsave?title='+additem_title+'&desc='+additems_notes+'&categoryname='+ categoryname +'&siteurl='+encodeURIComponent(text_url)+'&imageurl='+ image_name+'&additem_prices='+ additem_prices;
									$.ajax({
									      url: urls,
									      type: "GET",
									      dataType: "html",
...
function itemcou1(id){
	...
	//alert(likebtncnt);
		...
		if(loguserid == 0){
			...
			$.post(BaseURL+'mobile/userlike', {"itemid":id},
				...
				//$("#itemff"+id).html('<img src="'+BaseURL+'images/menu/fantacydbtn.png">');
				$("#load"+id).hide();
				$("#im"+id).attr("src",BaseURL+"images/logo/fantacyd.png");
				$(".itemdd"+id).css("color","#188EE6");
				$(".itemdd"+id).html(likedbtncnt);
				$('#ddheart'+id).addClass('heart');
				$('#ddheart'+id).removeClass('heart_empty');
				$('#dd'+id).removeClass('fantacy ui-link');
			...
			else if(fantacyval==likedbtncnt)
				{
				$.post(BaseURL+'mobile/userUnlike', {"itemid":id},
						function(datas) {
						datas = eval(datas);
						...
						//$("#itemff"+id).html('<img src="'+BaseURL+'images/menu/fantacybtn.png">');
						$("#im"+id).attr("src",BaseURL+"images/logo/fantacy.png");
						$(".itemdd"+id).css("color","#000000");
						$(".itemdd"+id).html(likebtncnt);
						$("#load"+id).hide();
						$('#ddheart'+id).addClass('heart');
						$('#ddheart'+id).removeClass('heart_empty');
	...
	var BaseURL=getBaseURL();
		...
		$('#showprofile').hide();
		   ...
		   $("#fav_countsvv").text(fav_counts);
			...
			var urlss = BaseURL+'mobile/listing/'+id +'/'+ title;
			//alert(urlss);
			...
			$('#share-social').hide();
	         ...
	         /* var usernames = $('#user_n'+id).attr("usernameval");
			   $("#usernames_popup").text(usernames);*/
				...
				var urlss = BaseURL+'mobile/people/'+uname;
				//alert(urlss);
	...
	function showdescription (id) {
		//alert(id);
		var baseurl = getBaseURL()+'mobile/viewitemdesc/'+id;
		$.ajax({
		      url: baseurl,
		      type: "post",
		      dataType: "html",
	...
	function hidedescription (id) {
	 ...
	 function indcall(event) {
		...
		window.location = baseurl+"mobile/searches/"+searchString;
	...
	function shownoti()
	{
		...
		if(pushnoii){
			pushnoii=false; 
  			//alert(loguserid);
  			$.ajax({
  		      url: BaseURL+"mobile/getpushajax/",
  		      type: "post",
	...
	function showcarthov()
	{
		...
		if(cartnoii){
			cartnoii=false; 
  			$.ajax({
  		      url: BaseURL+"mobile/cartmousehover/",
  		      type: "post",
  		      //data : { 'loginuserid': loguserid},
  		      dataType: "html",
  				beforeSend: function() {
	  ...
	  if(navigator.geolocation) {
	    navigator.geolocation.getCurrentPosition(function(position) {
	      var pos = new google.maps.LatLng(position.coords.latitude,
			...
			window.location.href=BaseURL+"mobile/nearme?lat="+lat+"&long="+lon+"&kilometer="+kilometer;
