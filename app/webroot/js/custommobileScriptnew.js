	...
	if(loguserid == 0){
		...
		if(confirm("Report this as inappropriate or broken ?")) {
			$.ajax({
			      ...
			      success: function(responce){
			    	  var data = '<p id="unreportflag" style="color:#979797;font-size:15px;"><img src="'+baseurl+'images/menu/flag.png"><span style="margin-left:8px;">Undo reporting</span>'+
			    	  '<img class="reportloader" style="display:none;" src="'+baseurl+'images/loading.gif" alt="loading..." />'+
					  '</p>';
...
$(document).on('click','#unreportflag',function(){
	...
	if(loguserid == 0){
		...
		if(confirm("Cancel Report this ?")) {
			$.ajax({
			      url: baseurl+'mobile/undoreportitem',
			      type: "GET",
			      data: { 'itemid':itemid, 'userid':loguserid},
			      beforeSend: function () {
			    	  $('.reportloader').show();
			      ...
			      success: function(responce){
			    	  var data = '<p id="reportflag" style="color:#979797;font-size:15px;"><img src="'+baseurl+'images/menu/flag.png"><span style="margin-left:8px;">Report as inappropriate</span>'+
			    	  '<img class="reportloader" style="display:none;" src="'+baseurl+'images/loading.gif" alt="loading..." />'+
					  '</p>';
			    	  $('.reportitem').html(data);
			      }  
...
$('#passw, #confirmpass').keyup(function() {
...
$('#phne, #person_phone_number, .order,#userentercreditamt, #zipcodegg, #telephonegg, #ggamt').keyup(function() {
    var $th = $(this);
   ...
   if(e.keyCode == 13 && str != '') {
	   var selectedCategory = $('#sub-category').val();
		...
		$.ajax({
		      ...
		      success: function(responce){
		          $('.shopcont').html(responce).trigger("create");
...
$('#searchstring').keyup(function(e) {
   ...
   if(e.keyCode == 13 && str != '') {
	   var selectedCategory = $('#sub-category1').val();
		if (selectedCategory == '') {
			selectedCategory = $('#currentCatPath').val();
		}
		var priceRange = $('#price-range1').val();
		var color = $('#color-filter1').val();
		var sortPrice = $('#sort-by-price1').val();
		var searchKey = str;
		var baseurl = getBaseURL()+"mobilecategories/getItemByCategory";
		var categoryId = $('#hiddencatvalue').val();
...
$('#fullname, #country, #state, #city, #merchant_name, #loc, #name').click(function() {
    var $th = $(this);
...
/*$('#updatelist').click(function(){ alert("hai");
	$("#myPopup").popup( "close" );
	...
	selectedCategory = selectedCategory.replace("mobile/","");alert(selectedCategory);
	var priceRange = $('#price-range').val();
	var color = $('#color-filter').val();
	var sortPrice = $('#sort-by-price').val();
...
selectedCategory = selectedCategory.replace("mobile/","");
var priceRange = $('#price-range').val();
var color = $('#color-filter').val();
var sortPrice = $('#sort-by-price').val();
var searchKey = $('.search-string').val();;
...
function updaterecomend1(){ 
	...
	selectedCategory = selectedCategory.replace("mobile/","");
	var priceRange = $('#price-range1').val();
	var relation = $('#relation-filter1').val();
	var gender = $('#sort-by-gender1').val();
	var baseurl = getBaseURL()+"mobilecategories/getItemByRelation";
	var categoryId = $('#hiddencatvalue').val();
...
function cartshipping(id,shopid) {
	...
	$.ajax({
	      ...
	      success: function(responce){
	    	  ...
	    	  if (obj[0] != null) {
	    	 $(shipaddrid).html(obj[0]).trigger("create"); 
	    	 }
	      } 
	}); 
}
	 ...
	 var iChars = "@";
	    ...
	    for (var i = 0; i < searchString.length; i++) {
	       if (iChars.indexOf(searchString.charAt(i)) != -1) {
	    	   ...
	    	   $.ajax({
	   			url: baseurl+'mobile/ajaxUserAuto',
	   			type: "post",
...
function deactiateac(id){
	var BaseURL=getBaseURL();
		...
		if (x){
			$.post(BaseURL+'mobile/deactivateacc', {"userid":id},
					function(datas) {
					window.location.href=BaseURL;
					}
				);
...
function checkcoupon(count,userId,shopId){
	...
	if(coupon_value==''){
		...
		var baseurlforcoupon = getBaseURL()+"mobile/entercouponvalue";
...
function Checkgiftcard(merchantId,shopId){
	...
	if(giftcard_value==''){
		...
		var baseurlforgiftcard = getBaseURL()+"mobile/entergfcardvalue";
