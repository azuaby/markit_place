...
var wantitajax = 1, ownitajax = 1, invajax = 0, sendmsgajax = 1, ggdeleteajax = 1;
var allcatvaluess = [];
...
$(document).ready(function () {
    ...
    $('#full-description').live('click',function(){
    	$(".thing-description").addClass('open');
    	$(".more-enable").html('<a href="javascript:void(0);" id="less-decription">less</a>');
    });

    $('#less-decription').live('click',function(){
    	$(".thing-description").removeClass('open');
...
$(".list-box li").live("click",function() {
...
$('#ggift').live('click' , function(){
	...
	if (sizestat == 1){
		...
		var qty = $("#qty_opt").val();
		if (size == ''){
			$('.addcarterror').html("Please Select Size");
			return false;
		}
	}else{
		var qty = $("#qty_opt").val();
		if (qty == ''){
			$('.addcarterror').html("Please Select Quantity");
			return false;
		}
	}
	//alert('working111');
...
$('#gglistcancel').live('click' , function(){
	var lastestidgg = $('#lastestidgg').val();
	if (lastestidgg != "" && ggdeleteajax == 1){
		ggdeleteajax = 0;
		$.ajax({
		  url: getBaseURL()+'ggdelete/'+lastestidgg,
		  type: "GET",
		  dataType: "text",
		  success: function(responce){
			$('#popup_container').hide();
...
$('#queryaboutitem').live('change', function(e) {
    if($(this).val() == 'Others'){
...
$('#btn-browses-cs').live('click' , function(){
	$('#popup_container').fadeIn();
	$('#popup_container').css({"opacity":"0"});
	$('.contact-seller').css({'margin':'350px auto 0'});
	$('.contact-seller').fadeIn('slow');
	$('#share-social').hide();
	$('.share-thing').hide();
	$('#videourrls').hide();
	$('#add-to-list-new').hide();
...
$('#phne, #person_phone_number, .order, #zipcodegg, #telephonegg, #ggamt').live('keyup', function() {
    var $th = $(this);
    $th.val( $th.val().replace(/[^0-9]/g, function(str) { return ''; } ) );
});
$('#userentercreditamt').live('keyup', function() {
    var $th = $(this);
    $th.val( $th.val().replace(/[^0-9.]/g, function(str) { return ''; } ) );
});
...
function shippingEdit(id) {
	var baseurl = getBaseURL();
	baseurl += "addshipping/"+id;
	window.location = baseurl;
}

function shippingdefault(id) {
	var baseurl = getBaseURL();
	baseurl += "users/defaultshipping";
	var trid = ".default"+id;
	$.ajax({
		url:baseurl,
		type:"post",
...
function cartshipping(id,shopid) {
	var baseurl = getBaseURL();
	baseurl += "paypals/changeshipping";
...
function validaddship() {
	...
	var state = $('#state-selectbx').val();
	var add1 = $('#add1').val();
	var city = $('#area-selectbx').val();
	var zip = $('#zip').val();
	var phone = $('#phne').val();
	...
	if (country == '') {
		$("#alert_cntry").show();
		$("#badMessage").hide();
		$('#alert_cntry').text('Enter your country');
		$('#countrys').focus()
	...
	if (state == '') {
		...
		$('#state-selectbx').focus()
		$('#state-selectbx').keydown(function() {
			$('#alert_state').hide();
		})
		return false;
	}
	...
	if (city == '') {
		$("#alert_area").show();
		$("#badMessage").hide();
		$('#alert_area').text('Enter your  city');
		$('#area-selectbx').focus()
		$('#area-selectbx').keydown(function() {
			$('#alert_area').hide();
		})
		return false;
	}
...
function userinfo() {
	...
	var country = $('#countrys').val();
	var state = $('#state-selectbx').val();
	var area = $('#area-selectbx').val();
	var block = $('#block').val();
	var street = $('#street').val();
	var bilding = $('#bild_no').val();
	var mobile = $('#mobile').val();
	var email = $('#email').val();
	var byear = $('#birthday_year').val();
	var bmonth = $('#birthday_month').val();
	var bday = $('#birthday_day').val();

	if (name == '') {
		$("#alert_FName").show();
		$("#badMessage").hide();
		$('#alert_FName').text('First name is required');
		$('#name').focus()
	...
	if (lastname == '') {
		$("#alert_LName").show();
		$("#badMessage").hide();
		$('#alert_LName').text('Enter your Last name');
		$('#lastname').focus()
	...
	if (country == '') {
		$("#alert_cntry").show();
		$("#badMessage").hide();
		$('#alert_cntry').text('Select your country');
		$('#countrys').focus()
	...
	if (state == '') {
		$("#alert_state").show();
		$("#badMessage").hide();
		$('#alert_state').text('Select your state');
		$('#state-selectbx').focus()
		$('#state-selectbx').click(function() {
			$('#alert_state').hide();
		})
		return false;
	}
	if (area == '') {
		$("#alert_area").show();
		$("#badMessage").hide();
		$('#alert_area').text('Select  your  Area');
		$('#area-selectbx').focus()
		$('#area-selectbx').click(function() {
			$('#alert_area').hide();
		})
		return false;
	}
	if (block == '') {
		$("#alert_block").show();
		$("#badMessage").hide();
		$('#alert_block').text('Select  your  block No.');
		$('#block').focus()
	...
	if (street == '') {
		$("#alert_street").show();
		$("#badMessage").hide();
		$('#alert_street').text('Select  your  street');
		$('#street').focus()
	...
	if (bilding == '') {
		$("#alert_bilding").show();
		$("#badMessage").hide();
		$('#alert_bilding').text('Select  your  House/Building No.');
		$('#bild_no').focus()
		$('#bild_no').keydown(function() {
			$('#alert_bilding').hide();
		})
		return false;
	}
	if (mobile == '') {
		$("#alert_Mobile").show();
		$("#badMessage").hide();
		$('#alert_Mobile').text('Enter your Mobile No.');
		$('#mobile').focus()
	...
	if (email == '') {
		$("#alert_Email").show();
		$("#badMessage").hide();
		$('#alert_Email').text('Enter your  Email');
		$('#email').focus()
	...
	if (byear == 0) {
		$("#alert_byear").show();
		$("#badMessage").hide();
		$('#alert_byear').text('Enter your  Birthday Year');
		$('#birthday_year').focus()
	...
	if (bmonth == 0) {
		$("#alert_bmonth").show();
		$("#badMessage").hide();
		$('#alert_bmonth').text('Enter your  Birthday Month');
	...
	if (bday == 0) {
		$("#alert_bday").show();
		$("#badMessage").hide();
		$('#alert_bday').text('Enter your  Birthday Date');
		$('#birthday_day').focus()
		$('#birthday_day').click(function() {
			$('#alert_bday').hide();
		})
		return false;
	}
	if(!(isValidEmailAddress(email))){
		$("#alert_Email").show();
		$("#badmessage").hide();
		$('#alert_Email').text('Enter a valid email');
			$('#email').focus()
		$('#email').keydown(function() {
			$('#alert_Email').hide();
		});
...
function checkcoupon(count,userId,shopId){
	...
	if(coupon_value==''){
		...
		var baseurl = getBaseURL()+'checkcouponcode?coupon_value='+coupon_value+'&userid='+userId+'&shopid='+shopId;
		...
		if (couajax == 0) {
			...
			$.ajax({
				  ...
				  if(responce == 4){
				        	$('#couponsUsed').show();
				      		setTimeout(function(){$('#couponsUsed').fadeOut();}, 3000); 
				      		$('#loadingimgs').hide();
			          }
...
function change(ItemId) {
	...
	$.ajax({
		...
		success : function(msg) {
			document.getElementById('show_Userurl').src=baseurl+'media/avatars/thumb350/usrimg.jpg';
			$('#loadingimgg').hide();
			if(img!='usrimg.jpg'){
...
function validggusersave() {
	...
	var baseurl = getBaseURL()+'ggusersave?recipient='+recipient+'&name='+name+
				...
				'&image='+image+'&size='+size+'&qty='+qty+'&lastestidgg='+lastestidgg;
...
function contributeChkOut(ggid, user) {
	var BaseURL=getBaseURL();
	if (user == 0){
		window.location.href=BaseURL+"login";
		return false;
...
function showcontactform(){
	...
	$('.contact-seller').show();
	setTimeout(function() {
		$('.contact-seller').css({'margin':'30px auto 0'});
	}, 30);
}
...
function validateaddcarts (){
	$('.addcarterror').html('');
	var sizeset = $('#sizeset').val();
	var qty = $('#qty_opts').val();
	if (sizeset == 1){
		var size = $('#size_opt').val();
		if (size == ''){
			$('.addcarterror').html('Select a size');
