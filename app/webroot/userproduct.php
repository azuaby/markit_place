	...
	if(($ext != '.JPG' && $ext != '.PNG' && $ext != '.JPEG' && $ext != '.GIF' && $ext != '.jpg' && $ext != '.png' && $ext != '.jpeg' && $ext != '.gif') || $fsize > 200*1024*1024){
		...
		if(isset($ftmp)){
			...
			<script>
				...
				par.getElementById('show_Userurl').src = '<?php echo $img_src; ?>';
				//document.getElementById('loadinguf').style.display="none";
...
function upload(){
	...
	//par.getElementById('show_Userurl').src = '<?php echo $site_url."img/loading.gif";?>';
	document.getElementById('loadinguf').style.display="block";
}
</script>
...
<form id="iform" name="iform" class="settings" action="" method="post" enctype="multipart/form-data">
	<div class="row file-holder1">
		<div class="file file-input-js-active">
			...
			<input type="text" id="input_value" value="Add Your Fashion" class="btn text file-input-value" readonly="readonly"/>			
			<div id="loadinguf" style="width: 15px; height: 15px; display: none; position: absolute; right: 12px; top: 11px;"><img src="<?php echo $site_url; ?>images/loading.gif" style="width:15px; height:15px;"></div>
		</div>
	</div>
</form>
