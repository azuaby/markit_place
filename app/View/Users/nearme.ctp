	...
	<?php } ?>	
		...
		if(!empty($items_data)){
			...
			foreach($items_data as $key=>$itms){
				...
				echo  '<em class="back"></em>';
				echo  "<img src='".$_SESSION['media_url']."media/items/thumb350/".$image_name."' style='background:#cdcdcd;' data-height=".$height." data-width=".$width.">";
				echo  '</span>';
...
$(window).scroll(function () {
 //if ($(document).height() - 50 <= $(window).scrollTop() + $(window).height()) {
  ...
  if (isPreviousEventComplete && isDataAvailable) {
    ...
    $.ajax({
      type: "GET",
      url: baseurl+'getMorenearme?startIndex=' + sIndex + '&offset=' + offSet + '&followid='+followid+'&loadmoretab='+loadmoretab+'&lat=<?php echo $lat; ?>&long=<?php echo $lng; ?>&kilometer=<?php echo $_GET['kilometer']; ?>',
      beforeSend: function () {
			$('#infscr-loading').show();
		},
	  dataType: 'html',
...
//alert();
	...
	function switchTab(tab){
		...
		if(!(result=$.jStorage.get('first-'+tab))){			
			$.ajax({
				url : baseurl+'changeTab?feed='+tab+'&lat=<?php echo $lat; ?>&long=<?php echo $lng; ?>&kilometer=<?php echo $_GET['kilometer']; ?>',
				dataType : 'html',
				beforeSend : function () {
					$(".stream").fadeOut("fast");
					$('#index-loading').show();
