	...
	<div class="box span12">
	    ...
	    echo "<div class='containerdiv'>";
            ...
            ?>
                ...
                <label><?php echo __('Zone Name');?></label>
				...
				<select id="zone_name" name="zone_name" class="selectboxdiv" style="width: 280px ! important;">
					...
					<?php $selected = ''; 
							foreach ($arealist as $country) { 
								if (isset($getchargeval)  && $country['Area']['area'] == $getchargeval['Deliverycharge']['name'])
									$selected = 'selected';
					?>
					<option value="<?php echo $country['Area']['area'];?>" <?php echo $selected;?>><?php echo $country['Area']['area']; ?></option>
					<?php $selected = '';} ?>
				</select>
				</div>
