...
@$hostname = $_SESSION['media_host_name'];
	...
	echo "<div class='containerdiv'>";
		echo $this->Form->Create('Item',array('url'=>array('controller'=>'admins','action'=>'/add_item'),'id'=>'itemform','onsubmit'=>'return validate()'));
		...
		?>
			...
			?>
							<div id='categ-container-2' class='select-group inactive ' style='float:left;margin-left:20px;'>
								<label class="invisible">What type? <span class="optional"></span> </label>
								<select name="data['Item']['supersubcat']" id="categ-selectbx-2">
								</select>
								<?php
								echo '<br clear="all" /><div class="subcat-error adminitemerror" style="color:red"></div>';
								?>
							</div>
			...
			<?php
						...
						for($i=0;$i<5;$i++){
							echo "<div class='img_upld' style='width:20% !important;float:left;'>";
								...
								echo '<div class="venueimg"><iframe class="image_iframe" id="frame_'.$i.'" name="frame'.$i.'" src="'.$this->webroot.'imageupload.php?image='.$i.'&media_url='.$media.'&site_url='.$site.'&username='.$username.'&password='.$password.'&hostname='.$hostname.'" frameborder="0" height="40px" width="120px" scrolling="no" HSPACE=0 VSPACE=0 style="float:left;left: 0px;position: relative;"></iframe>';												
									...
									echo "</br><a href='javascript:void(0);' id='removeimg_".$i."' class='btn' style='display: none; margin-top: -15px; float: left;' onclick='removeimg(".$i.")'>Remove</a>";
								echo "</div>";
							echo "</div>";
						}
					echo "</div>";
				...
				echo "<input type='text' id='video' name ='data[Item][item_video]' class='input-xlarge itemvideo' maxlength='180' value='' placeholder='YouTube Url or YouTube video id' style='color:#555555! important;'>";
				...
				echo "<div class='input-group'>";
					...
					echo "<input type='text' id='title' name ='data[Item][item_title]' class='input-xlarge itemtitle' maxlength='180' value=''>";
				...
				echo "<div class='input-group'>";
					...
					echo $this->Form->input('item_description',array('type'=>'textarea','label'=>false,'id'=>'description','class'=>'input-xlarge','value'=>''));
                ...
                <div class="section-inner clear">
					<div class="input-group clear input-group-error" id="item-price">
						...
						<span class="price-input">
							<span><?php echo $_SESSION['default_currency_symbol'];?></span>
							<input type="text" value="" class="input-xlarge money text text-small" id="price" name="listing[price]"  maxlength = "5">
							<span><?php echo $_SESSION['default_currency_code'];?></span>
					...
					<div class="input-group clear" id="item-quantity">
						...
						<input type="text" value="1" maxlength="3" class="input-xlarge text text-small" id="quantity" name="listing[quantity]">
  ...
  <div style="" class="set-shipping-rates">
        <table class="tablesorter table table-striped table-bordered table-condensed" id="shpng_div">
					...
					<td id="<?php echo $cntryid[$cntry_code]; ?>">        
					 ...
					 <td>          
						<div class="input-group input-group-price price-input primary-shipping-price"> 
							...
							<input type="text" value="" id="shippingPrice" class="money text text-small input-small" name="country_shipping[<?php echo $cntryid[$cntry_code]; ?>][0][primary]">            
						</div>      
					</td>     
		 ...
		 <div class="ship-error adminitemerror" style="color:red"></div>
