...
App::uses('AppController', 'Controller');
	...
	class UsersController  extends AppController{
		...
		function index(){
			...
			if($user_level == 'god' || $user_level=="moderate"){
				$this->redirect('/admin/');
			}
	...
	function getMorePosts() {
		...
		function changeTab () {
			...
			//echo "Item<pre>";print_r($itemid);die;
			if (isset($_GET['feed']) && $_GET['feed'] == 'following') {
				...
				if (count($following) > 0){
					...
					if($setngs[0]['Sitesetting']['affiliate_enb']=='enable'){
						if(isset($itemid)){
							$followDetails = $this->Item->find('all',array('conditions'=>array('status <>'=>'draft','Item.user_id'=>$followingId,'Shop.shop_latitude BETWEEN ? AND ?' => array($LatS,$LatN) , 'Shop.shop_longitude BETWEEN ? AND ?' => array($LonW,$LonE)),'limit'=>'10','order'=>array('Item.id'=>'desc')));
						}else{
						...
						if(isset($itemid)){
							$followDetails = $this->Item->find('all',array('conditions'=>array('status'=>'publish','Item.user_id'=>$followingId,'Shop.shop_latitude BETWEEN ? AND ?' => array($LatS,$LatN) , 'Shop.shop_longitude BETWEEN ? AND ?' => array($LonW,$LonE)),'limit'=>'10','order'=>array('Item.id'=>'desc')));
						}else{
				...
				if($setngs[0]['Sitesetting']['affiliate_enb']=='enable'){
					if(isset($itemid)){
						$followDetails = $this->Item->find('all',array('conditions'=>array('Item.status <>'=>'draft','Item.featured'=>'1','Shop.shop_latitude BETWEEN ? AND ?' => array($LatS,$LatN) , 'Shop.shop_longitude BETWEEN ? AND ?' => array($LonW,$LonE)),'limit'=>'20','order'=>array('Item.id'=>'desc')));
					}else{
					...
					if(isset($itemid)){
						$followDetails = $this->Item->find('all',array('conditions'=>array('Item.status'=>'publish','Item.featured'=>'1','Shop.shop_latitude BETWEEN ? AND ?' => array($LatS,$LatN) , 'Shop.shop_longitude BETWEEN ? AND ?' => array($LonW,$LonE)),'limit'=>'20','order'=>array('Item.id'=>'desc')));
					}else{
				...
				if($setngs[0]['Sitesetting']['affiliate_enb']=='enable'){
					if(isset($itemid)){
						$followDetails = $this->Item->find('all',array('conditions'=>array('status <>'=>'draft','Shop.shop_latitude BETWEEN ? AND ?' => array($LatS,$LatN) , 'Shop.shop_longitude BETWEEN ? AND ?' => array($LonW,$LonE)),'limit'=>'20','order'=>array('Item.id'=>'desc')));
					}else{
					...
					if(isset($itemid)){
						$followDetails = $this->Item->find('all',array('conditions'=>array('status'=>'publish','Shop.shop_latitude BETWEEN ? AND ?' => array($LatS,$LatN) , 'Shop.shop_longitude BETWEEN ? AND ?' => array($LonW,$LonE)),'limit'=>'20','order'=>array('Item.id'=>'desc')));
					}else{
						$followDetails = $this->Item->find('all',array('conditions'=>array('status'=>'publish'),'limit'=>'20','order'=>array('Item.id'=>'desc')));
					}
			...
			if(!empty($this->request->data)){
				...
				if((!empty($email)) && (!empty($password))){
					...
					if(!empty($userdata)){
							...
							if ($userdata[0]['User']['activation'] == 1) {
								if($this->Auth->login($userdata)){
									...
									//}else{
										//$redirctt = $_SESSION['referal'];
									//}
									$this->redirect("$redirctt");	
								}
							} else {
								$this->Session->setFlash(__('Please activate your account by the email sent to you'), 'default', array(), 'bad');
								$this->redirect('/login');
							}
						} else {
						...
						$this->Session->setFlash(__('Please enter correct email and password'), 'default', array(), 'bad');
		...
		public function signup(){
			...
			if(!empty($this->request->data)){
				...
				$countryid = $this->request->data['cntry'];
				$countryModel = $this->Country->find('all',array('conditions'=>array('id'=>$countryid)));
				$countryname = $countryModel[0]['Country']['country'];
				...
				if($nmecounts > 0){
					...
					$this->request->data['User']['countrycode'] = $countryid;
					...
					if ($setngs[0]['Sitesetting']['signup_active'] == 'no') {
					...
					if ($setngs[0]['Sitesetting']['signup_active'] == 'yes') {
						...
						$this->Session->setFlash(__('An email was sent to your mail box, please activate your account and login.'));
						$this->redirect('/');
					}else{
						$this->Session->setFlash(__('Your account has been created, please login to your account'));
						$this->redirect('/login');
					}
		...
		public function forgotpassword(){
			...
			if(!empty($this->request->data)){
				...
				$name = $usr_datas['User']['first_name'];
				...
				if(!empty($reg_email)){
					...
					$this->Session->setFlash(__('Email id is not found Please register to our site.'));
					$this->redirect($this->referer());
				}
			}
		}
		...
		public function verification($userid = null){
			...
			if(!empty($userdetails)){
				...
				if($active == 0){
					...
					$welcome_email = $setngs[0]['Sitesetting']['welcome_email'];
					if($welcome_email=="yes")
					{
						...
						if($setngs[0]['Sitesetting']['gmail_smtp'] == 'enable'){
							...
							$this->Email->to = $emailaddress;
							$this->Email->subject = $setngs[0]['Sitesetting']['site_name']." – Congratulations, your new account is ready";
							$this->Email->from = SITE_NAME."<".$setngs[0]['Sitesetting']['noreply_email'].">";
							$this->Email->sendAs = "html";
							$this->Email->template = 'welcomeemail';		
							$this->set('name', $name);
							$this->set('urlname', $urlname);
							$this->set('email', $emailaddress);
							$this->set('username',$userdetails[0]['User']['first_name']);
							
							$this->Email->send();  
					
					}
						
					$this->Session->setFlash(__('Your account is Verified, please login and continue'));
					$this->redirect('/');
				}else if($active == 1){
					if($this->Auth->login($userdetails)){
						$cookie['email'] = $email;
					...
					$this->Session->setFlash(__('Your account was already Verified'));
					$this->redirect('/');
				}
			}else{
				$this->Session->setFlash(__('You are not authenticated.Please signup'));
				$this->redirect('/');
			}
		}
		...
		function userprofiles($name = null,$usrstates = null,$pagev = null){
			...
			if(!empty($flwrs)){
				...
				foreach($flwrscnt as $flwss){
					$flwrcntusrids[$flwss['Follower']['follow_user_id']] = $flwss['Follower']['follow_user_id'];
				}
			...
			if(!empty($follow)){
				...
				foreach($followcnt as $flcntt){
					$flwngcntusrids[] = $flcntt['Follower']['user_id'];
				}
			}
			
			$followerCount = $this->User->find('count',array('conditions'=>array('User.user_level <>'=>'god','User.activation <>'=>'0','User.id'=>$flwrcntusrids)));
			$followCount = $this->User->find('count',array('conditions'=>array('User.user_level <>'=>'god','User.activation <>'=>'0','User.id'=>$flwngcntusrids)));
			$this->set('totl_flwrs',$totl_flwrs);
			...
			if(isset($_REQUEST['wantit'])){
				$wantOwnModel = $this->Wantownit->find('all',array('fields'=>array('itemid'),'conditions'=>array('userid'=>$current_page_userid,'type'=>'want'),'limit'=>15));
				$wantModel = array();
				foreach($wantOwnModel as $want){
					$wantModel[] = $want['Wantownit']['itemid'];
				}
			...
			$wantCount = $this->Wantownit->find('count',array('conditions'=>array('userid'=>$current_page_userid,
							'type'=>'want')));
			$this->set('wantCount',$wantCount);
				
			if(isset($_REQUEST['ownit'])){
				$wantOwnModel = $this->Wantownit->find('all',array('fields'=>array('itemid'),'conditions'=>array('userid'=>$current_page_userid,'type'=>'own'),'limit'=>15));
				$ownModel = array();
				foreach($wantOwnModel as $own){
					$ownModel[] = $own['Wantownit']['itemid'];
				}
			...
			$OwnCount = $this->Wantownit->find('count',array('conditions'=>array('userid'=>$current_page_userid,
									'type'=>'own')));
			...
			if(isset($_REQUEST['lists'])){
				$itemListsAll = $this->Itemlist->find('all',array('conditions'=>array('user_id'=>$usr_datas['User']['id']),'limit'=>15, 'order'=>array('Itemlist.created_on'=>'desc')));
				$itemListsCount = $this->Itemlist->find('count',array('conditions'=>array('user_id'=>$usr_datas['User']['id'])));
		...
		function manageprofile($name = null){
			...
			if(empty($name)){
				$this->Session->setFlash(__('Sorry, something went wrong'));
				$this->redirect($this->referer());
			}
			...
			if(!$loguser[0]['User']['id']){
				$this->Session->setFlash(__('You are not authenticate to edit others profile.'));
				$this->redirect($this->referer());
			}
		...
		function addflw_usrs(){
			...
			if($flwalrdy > 0){
				...
				if ($userid != $logusrid){
					...
					if (!empty($userModel['User']['push_notifications'])){
						$pushSettings = json_decode($userModel['User']['push_notifications'],true);
						if ($pushSettings['frends_flw_push'] == 1){
							...
							foreach($userddett as $userdet){
								...
								if(isset($deviceTToken)){
									$messages = $logusername." is following you";
			...
			$user_email = $this->User->find('all',array('conditions'=>array('User.id'=>$userid)));
			$emailaddress = $user_email[0]['User']['email'];
			$name = $user_email[0]['User']['first_name'];
			$follow_status = $user_email[0]['User']['someone_follow'];
			...
			if($follow_status==1)
			{
				...
				$this->Email->to = $emailaddress;
				$this->Email->subject = $setngs[0]['Sitesetting']['site_name']." – ".$logusername." Following you on ".$setngs[0]['Sitesetting']['site_name'];
				$this->Email->from = SITE_NAME."<".$setngs[0]['Sitesetting']['noreply_email'].">";
				$this->Email->sendAs = "html";
				...
				$this->Email->send();			
					...
					$this->Session->setFlash(__('You cannot follow yourself'));
					$this->redirect('/');
				}
			}	
			die;
	...
	function peoplesearch($name = null){
			...
			$banner_datas = $this->Banner->find('first',array('conditions'=>array('Banner.banner_type'=>'findfriends')));
			$this->set('banner_datas',$banner_datas);			
			
			$followcnt = $this->User->findByUsernameUrl($name);
			...
			if(!empty($this->request->data)){
				...
				if(!empty($username_val)){
					...
					$people_details =  $this->User->find('all',array('conditions'=>array('User.user_level <>'=>'god','User.activation <>'=>'0','User.id <>'=>"'$flwngusrids'"),'limit'=>5,'order'=>array('User.id'=>'desc')));
				}
		//echo "<pre>";print_r($people_details);	die;
				foreach($people_details as $ppl_dtl){
					foreach($ppl_dtl['Itemfav'] as $ppl_dt){
				...
				$similaruser_details =  $this->User->find('all',array('conditions'=>array('User.user_level <>'=>'god','User.id <>'=>"'$flwngusrids'",'User.id'=>"'$similar_users'"),'order'=>array('User.id'=>'desc')));
		...
		function user_update(){
			...
			if(!empty($this->request->data)){
				...
				$this->loadModel('User');
				//echo "<pre>";print_r($this->request->data);die;
				$loguid=$loguser[0]['User']['id'];
				$this->request->data['Fashionuser']['user_id'] = $loguser[0]['User']['id'];
				$imgupfa=$this->request->data['Fashionuser']['userimage'] = $this->request->data['src'];
				$itid=$this->request->data['Fashionuser']['itemId'] = $this->request->data['ItemId'];
				$itemdet = $this->Item->findByid($itid);
				$emi = $this->Item->findByid($itid);
				$usid=$itemdet['Item']['user_id'];
				$itemname=$itemdet['Item']['item_title_url'];
				
				//print_r($usid);
				$buyid = $this->User->findByid($usid);
				$buyerid=$buyid['User']['email'];
				$buyername=$buyid['User']['first_name'];
				$buyerurl=$buyid['User']['username_url'];
				$selid = $this->User->findByid($loguid);
				$sellerid=$selid['User']['email'];
				$sellername=$selid['User']['first_name'];
				...
				if($this->request->data['src'] != 'usrimg.jpg' && $this->request->data['src'] != ''){
					...
					$this->Email->to = $buyerid;
					$this->Email->subject = $setngs[0]['Sitesetting']['site_name']." – There is a fashion uploaded on your product";
					
					$this->Email->from = SITE_NAME."<".$setngs[0]['Sitesetting']['noreply_email'].">";
					$this->Email->sendAs = "html";
					$this->Email->template = 'uploadfashionnew';
					$this->set('buyerid', $buyerid);
				$this->set('sellerid', $sellerid);
				$this->set('buyername', $buyername);
				$this->set('buyerurl', $buyerurl);
		...
		function purchaseditem () {
			...
			$userordersModel = $this->Dispute->find('all',array('conditions'=>array('uorderid'=>$orderid)));
			$userorderid = array();
			foreach ($userordersModel as $value) {
				$userorderid[] = $value['Orders']['orderid'];
			}
			
			
			$disp_data = $this->Dispute->find('all',array('conditions'=>array('uorderid'=>$orderid)));
			...
			$shortlist = $this->Dispute->find('all', array('conditions' => array('Dispute.uorderid' => $short_list_array)));
		...
		/**
			purchased item load more function ruban
		**/



			function getrecentpurchasebuyer(){
			//$orderDetails as $ky=>$orderDetail
			$this->autoLayout = FALSE;
			global $loguser;
			global $siteChanges;
			global $setngs;
	...
	function getmorecommentviewpurchase() {
			$this->autoLayout = FALSE;
			$this->autoRender = FALSE;
			global $loguser;
			global $siteChanges;
			...
			$ordersModel = $this->Orders->find('all',array('conditions'=>array('userid'=>$userid),'order'=>'orderid DESC','offset'=>$offset,'limit'=>'10'));
			$orderid = array();
			foreach ($ordersModel as $value) {
				$orderid[] = $value['Orders']['orderid'];
			}
...
/*****  user dispute *****/
			...
			global $setngs;
			$userid = $loguser[0]['User']['id'];
			$emailaddress = $loguser[0]['User']['email'];
			if(!$this->isauthenticated()){
				$this->redirect('/');
			}
			$this->loadModel('Orders');
			...
			if (!empty($this->request->data['Dispute'])){
				
				$or=$orderid;
				//$or=$this->request->data['Dispute']['order'];
				$orderModel = $this->Orders->findByorderid($orderid);
				$merchantid = $orderModel['Orders']['merchant_id'];
				$resol = $orderModel['Orders']['status'];
				$ordate = $orderModel['Orders']['orderdate'];
				$userModel = $this->User->findByid($userid);
				$merchantModel = $this->User->findByid($merchantid);
				$userEmail = $userModel['User']['email'];
				$merEmail = $merchantModel['User']['email'];
				
				$userName = $userModel['User']['first_name'].' '.$userModel['User']['last_name'];
				$merName = $merchantModel['User']['first_name'].' '.$merchantModel['User']['last_name'];
				$orderlist = $this->Order_items->findByorderid($orderid);
				$oname = $orderlist['Order_items']['itemname'];
				
				$buyerfirstname=$userModel['User']['first_name'];
				$buyerurl=$userModel['User']['username_url'];
				$sellerfirstname=$merchantModel['User']['first_name'];
				$sellerurl=$merchantModel['User']['username_url'];
				
				
				$uids=$this->request->data['Dispute']['userid']=$userid;
				$sids=$this->request->data['Dispute']['selid']=$merchantid;
				$or=$this->request->data['Dispute']['uorderid'] = $orderid;
				$una=$this->request->data['Dispute']['uname']=$userName;
				$uema=$this->request->data['Dispute']['uemail']=$userEmail;
				$sna=$this->request->data['Dispute']['sname']=$merName;
				$sema=$this->request->data['Dispute']['semail']=$merEmail;
				$plm=$this->request->data['Dispute']['uorderplm'] = $this->request->data['Dispute']['plm'];
				$ms=$this->request->data['Dispute']['uordermsg'] = $this->request->data['Dispute']['msg'];
				$orname=$this->request->data['Dispute']['uorderstatus']=$oname;
				$oda=$this->request->data['Dispute']['orderdatedisp']=$ordate;
				$neia="Buyer";
				$cre=$this->request->data['Dispute']['create']= $neia;
				$resolved="Pending";
				$cre=$this->request->data['Dispute']['resolvestatus']= $resol;
				...
				$this->Dispute->save($this->request->data);
				 $dis =  $this->Dispute->getLastInsertID(); 
				 
				 
				 $cuids=$this->request->data['Dispcon']['user_id']=$userid;
				 $gli=$this->request->data['Dispcon']['dispid']=$dis;
				 $gor=$this->request->data['Dispcon']['order_id'] = $or;
				 $gms=$this->request->data['Dispcon']['message'] = $ms;
				 $merid=$this->request->data['Dispcon']['msid'] = $sids;
				 ...
				 $this->set('buyerfirstname',$buyerfirstname);
				 $this->set('buyerurl',$buyerurl);
				 $this->set('sellerfirstname',$sellerfirstname);
				 $this->set('sellerurl',$sellerurl);
				 $this->set('gor',$gor);
				 $this->set('gli',$gli);
		...
		function disputepro(){
				...
				global $setngs;
				$userid = $loguser[0]['User']['id'];
				 $firstname= $loguser[0]['User']['first_name'];
				...
				foreach ($msgel as $key => $msg){
					$messagesel[$key]['userid'] = $msg['Disputes']['userid'];
					$messagesel[$key]['uorderstatus'] = $msg['Disputes']['uorderstatus'];
					$messagesel[$key]['uorderplm'] = $msg['Disputes']['uorderplm'];
				
					$messagesel[$key]['uorderid'] = $msg['Disputes']['uorderid'];
					$messagesel[$key]['uordermsg'] = $msg['Disputes']['uordermsg'];
				}
				...
				if(isset($_REQUEST['seller'])){
					...
					$msbuyer = $this->Disputes->find('all',array('conditions'=>array('selid'=>$userid),'order'=>array('disid'=>'desc')));
					foreach ($msbuyer as $key => $msgbuyer){
						$messagebuyer[$key]['selid'] = $msgbuyer['Disputes']['selid'];
						$messagebuyer[$key]['uorderstatus'] = $msgbuyer['Disputes']['uorderstatus'];
						$messagebuyer[$key]['uorderplm'] = $msgbuyer['Disputes']['uorderplm'];
						$messagebuyer[$key]['userid'] = $msgbuyer['Disputes']['userid'];
						$messagebuyer[$key]['uorderid'] = $msgbuyer['Disputes']['uorderid'];
						$messagebuyer[$key]['uordermsg'] = $msgbuyer['Disputes']['uordermsg'];
					}
			...
			function disputemessage($orderid){
				$this->layout = 'frontlayout';
				$this->set('title_for_layout','- '.ucwords($name));
				global $loguser;
				global $siteChanges;
				...
				$orderdet = $this->Dispute->findByuorderid($orderid);
				$buyerModel = $this->User->findByid($login);
				$buyerName = $buyerModel['User']['username'];
				$buyeremail = $buyerModel['User']['email'];
				
				$merchantModel = $this->User->findByid($merchantid);
				$sellerName = $merchantModel['User']['first_name'];
				$selleremail= $merchantModel['User']['email'];
				$buyerfirstname = $buyerModel['User']['first_name'];
				$msgel = $this->Dispcon->find('all',array('conditions'=>array('order_id'=>$orderid,user_id=>$userid),'order'=>array('dcid'=>'desc')));
				
				foreach ($msgel as $key => $msg){
					$messagedisp[$key]['user_id'] = $msg['Dispcon']['user_id'];
					$messagedisp[$key]['commented_by'] = $msg['Dispcon']['commented_by'];
					$messagedisp[$key]['date'] = $msg['Dispcon']['date'];
					$messagedisp[$key]['msid'] = $msg['Dispcon']['msid'];
					$messagedisp[$key]['order_id'] = $msg['Dispcon']['order_id'];
					$messagedisp[$key]['message'] = $msg['Dispcon']['message'];
				}
				...
				$cuids=$this->request->data['Dispcon']['user_id']=$userid;
				$gli=$this->request->data['Dispcon']['dispid']=$dis;
				$gor=$this->request->data['Dispcon']['order_id'] = $orderid;
				$gmsss=$this->request->data['Dispcon']['message'] = $this->request->data['Dispute']['msg'];
				$merid=$this->request->data['Dispcon']['msid'] = $orderdet['Dispute']['selid'];
				$liid=$this->request->data['Dispcon']['dispid'] = $orderdet['Dispute']['disid'];
				$da=$this->request->data['Dispcon']['date']= time();
				$nei="Buyer";
				$cre=$this->request->data['Dispcon']['commented_by']= $nei;
				...
				$this->set('gmsss',$gmsss);
				$this->set('gor',$gor);
				$this->set('gli',$gli);
				$this->set('liid',$liid);
				$this->set('buyerfirstname',$buyerfirstname);
				$this->set('sellerName',$sellerName);
				//print_r($selleremail);print_r($buyeremail);die;
				...
				$this->Email->subject = $setngs[0]['Sitesetting']['site_name']." – There is a response on the dispute #".$liid;
				$this->Email->from = SITE_NAME."<".$setngs[0]['Sitesetting']['noreply_email'].">";
				$this->Email->sendAs = "html";
	...
	function getbuyercmnt(){
		...
		function disputesellermsg($orderid){
					$this->layout = 'frontlayout';
					$this->set('title_for_layout','- '.ucwords($name));
					global $loguser;
					global $siteChanges;
					...
					$sellerName = $merchantModel['User']['first_name'];
					$selleremail = $merchantModel['User']['email'];
					$buyerefirstname = $buyerModel['User']['first_name'];
					
					$orderdet = $this->Dispute->findByuorderid($orderid);
					$umer=$orderdet['Dispute']['user_id'];
					$msgel = $this->Dispcon->find('all',array('conditions'=>array('order_id'=>$orderid,'msid'=>$userid),'order'=>array('dcid'=>'desc')));
					...
					foreach ($msgel as $key => $msg){
						...
						$messagedisp[$key]['date'] = $msg['Dispcon']['date'];
						$messagedisp[$key]['commented_by'] = $msg['Dispcon']['commented_by'];
						//$message[$key]['commented_by'] = $msg['Dispcon']['user_id'];
							
						$messagedisp[$key]['order_id'] = $msg['Dispcon']['order_id'];
						//$message[$key]['message'] = $msg['Dispute']['message'];
					}
					$this->set('messagedisp',$messagedisp);
					$this->set('orderdet',$orderdet);
					$this->set('buyerModel',$buyerModel);
					...
					if(isset($_REQUEST['selconver'])){
						$orderdet = $this->Dispute->findByuorderid($orderid);
						$msgel = $this->Dispcon->find('first',array('conditions'=>array('order_id'=>$orderid)));
						 $msgel['Dispcon']['user_id'];
						$orderid;
						
						$cuids=$this->request->data['Dispcon']['user_id']=$msgel['Dispcon']['user_id'];
						//$gli=$this->request->data['Dispcon']['dispid']=$dis;
						$gor=$this->request->data['Dispcon']['order_id'] = $orderid;
						$gmsss=$this->request->data['Dispcon']['message'] = $this->request->data['Dispute']['msg'];
						$merid=$this->request->data['Dispcon']['msid'] = $orderdet['Dispute']['selid'];
					...
					$this->set('firstname',$firstname);
						...
						$this->Email->subject = $setngs[0]['Sitesetting']['site_name']." – There is a response on your dispute #".$liid;
						$this->Email->from = SITE_NAME."<".$setngs[0]['Sitesetting']['noreply_email'].">";
						$this->Email->sendAs = "html";
						$this->Email->template = 'disputereply';
						$this->set('UserId', $cuids);
						$this->set('OrderId', $gor);
						//$this->set('Problem', $plm);
						$this->set('Message',$gmsss);
						...
						$this->redirect(array('controller' => '/', 'action' => 'disputeBuyer', $orderid));
					}	
...
function getoldorderseller(){
		...
		function getmoreoldorderseller() {
			$this->autoLayout = FALSE;
			$this->autoRender = FALSE;
			global $loguser;
			global $siteChanges;
			...
			$ordersModel = $this->Orders->find('all',array('conditions'=>array('merchant_id'=>$userid),'order'=>'orderid DESC','offset'=>$offset,'limit'=>'10'));
			$orderid = array();
			foreach ($ordersModel as $value) {
				$orderid[] = $value['Orders']['orderid'];
			}
...
function getrecentorder(){
		...
		function getmorerecentorder() {
			...
			//$ordersModel = $this->Orders->find('all',array('conditions'=>array('merchant_id'=>$userid,'orderdate >'=>$timeline,'status <>'=>$status),'order'=>array('orderid'=>'DESC'),array('offset'=>$offset,'limit'=>'10')));
			$orderid = array();
			foreach ($ordersModel as $value) {
			...
			$this->set('order_id',$order_id);
			$this->set('userid',$userid);
			$this->render('getrecentorder');
		}
		


		function getrecentdispallbuyer(){
			$this->autoLayout = FALSE;
			global $loguser;
			global $siteChanges;
			global $setngs;
			...
			$messagesel = $this->Disputes->find('all',array('conditions'=>array('userid'=>$userid),'offset'=>$currentcont,'limit'=> '40'));
			//print_r($ordercommentsModel);die;
			if (!empty($messagesel)){
		...
		function getmorecommentview() {
			...
			$messagesel = $this->Disputes->find('all',array('conditions'=>array('userid'=>$userid),'order'=>'disid DESC','offset'=>$offset,'limit'=>'3'));
			if (!empty($messagesel)){
				$latestcount = $currentcont + count($messagesel);
			...
			$this->set('order_id',$order_id);
			$this->set('userid',$userid);
			$this->render('getrecentdispallbuyer');
		}
		



		function getrecentdispallseller(){
			$this->autoLayout = FALSE;
			global $loguser;
			global $siteChanges;
			global $setngs;
			...
			$messagebuyer = $this->Disputes->find('all',array('conditions'=>array('selid'=>$userid),'offset'=>$currentcont,'limit'=> '40'));
			//print_r($ordercommentsModel);die;
			if (!empty($messagebuyer)){
				$latestcount = $currentcont + count($messagebuyer);
		...
		function getmorecommentviewseller() {
			$this->autoLayout = FALSE;
			$this->autoRender = FALSE;
			global $loguser;
			global $siteChanges;
			...
			$messagebuyer = $this->Disputes->find('all',array('conditions'=>array('selid'=>$userid),'order'=>'disid DESC','offset'=>$offset,'limit'=>'3'));
			if (!empty($messagebuyer)){
				$latestcount = $currentcont + count($messagebuyer);
			...
			$this->set('order_id',$order_id);
			$this->set('userid',$userid);
			$this->render('getrecentdispallseller');
		}
		...
		function oldmyorders () {
			...
			$timeline = strtotime('-1 month');
			$ordersModel = $this->Orders->find('all',array('conditions'=>array('merchant_id'=>$userid,'orderdate <' =>$timeline),'order'=>array('orderid'=>'desc')));
			$orderid = array();
			foreach ($ordersModel as $value) {
				$orderid[] = $value['Orders']['orderid'];
			}
		...
		function orderstatus(){
			...
			$orderModel = $this->Orders->findByorderid($orderid);
			$orderModl = $this->Orders->find('first',array('conditions'=>array('orderid'=>$orderid)));
			$orderitemModel = $this->Order_items->find('all',array('conditions'=>array('orderid'=>$orderid)));
			...
			$usershipping_addr = $this->Shippingaddresses->findByShippingid($orderModl['Orders']['shippingaddress']);
			if($status == 'Delivered'){
				$this->Orders->updateAll(array('status' => "'$status'", 'status_date' => "'$statusDate'", 'deliver_date' => "'$deliverDate'"), array('orderid' => $orderid));
				
				
			$user_name = $this->User->find('all',array('conditions'=>array('User.id'=>$orderModel['Orders']['merchant_id'])));
			$username = $user_name[0]['User']['username'];	
			$emailaddress = $user_name[0]['User']['email'];
			
			$buyer_name = $this->User->find('all',array('conditions'=>array('User.id'=>$orderModel['Orders']['userid'])));
			$buyername = $buyer_name[0]['User']['username'];
			$buyerurl = $buyer_name[0]['User']['username_url'];
		...
		$this->set('access_url',SITE_URL."login");
		$this->set('orderdate',$orderModl['Orders']['orderdate']);
		$this->set('usershipping_addr',$usershipping_addr);
		$this->set('totalcost',$orderModl['Orders']['totalcost']);
		$this->set('currencyCode',$orderModl['Orders']['currency']);
...
$username = $loguser[0]['User']['username'];
			...
			$email_address = $this->User->find("all",array("conditions"=>array('User.id'=>$emailId)));
			$emailaddress = $email_address[0]['User']['email'];	
			$name = $email_address[0]['User']['username'];	
		if($setngs[0]['Sitesetting']['gmail_smtp'] == 'enable'){
			$this->Email->smtpOptions = array(
				'port' => $setngs[0]['Sitesetting']['smtp_port'],
				'timeout' => '30',
		...
		$this->set('message',$_POST['comment']);
		$this->set('access_url',SITE_URL."login");
		...
		function updatetrackingdetails(){
			...
			$statusDate = time();
			$this->Orders->updateAll(array('status' => "'Shipped'", 'status_date' => "'$statusDate'"), array('orderid' => $_POST['orderid']));
			$this->redirect('/orders');
		}
		...
		function user_settings(){
			...
			$this->loadModel('Area');
			$this->loadModel('Allstate');
			$this->loadModel('Allarea');
			...
			if(!empty($this->request->data)){
				...
				$areaid = $this->request->data['area'];
				$areaModel = $this->Allarea->findById($areaid);
				$areaname = $areaModel['Allarea']['area'];

				$stateid = $this->request->data['state'];
				$stateModel = $this->Allstate->findById($stateid);
				$statename = $stateModel['Allstate']['state'];

				
//				$areacode = $this->request->data['area'];
//				$areaModel = $this->Country->findByid($areacode);
//				$areaname = $areaModel['Country']['country'];
				...
				$mobile_no = $this->request->data['mobile'];
				$mobile_code = $this->request->data['mcode'];
				$mobile = $mobile_code.'-'.$mobile_no;
				...
				$this->request->data['User']['country'] = $countryname;
				$this->request->data['User']['countrycode'] = $countrycode;

    			$prefix = $this->Allowedcountries->tablePrefix;
    			$prefixs = $this->State->tablePrefix;
    			$prefixa = $this->Area->tablePrefix;
                $userDetails = $this->Allowedcountries->query("SELECT * FROM ".$prefix."allowed_countries Where code = '{$countrycode}'");
                $userSDetails = $this->State->query("SELECT * FROM ".$prefixs."states Where state = '{$statename}'");
                $userADetails = $this->Area->query("SELECT * FROM ".$prefixa."areas Where area = '{$areaname}'");

                if($userDetails == NULL || $userSDetails == NULL || $userADetails == NULL){
                    $NoteMsg = "markit doesn’t delivery to added address";
    				$this->Session->setFlash(__($NoteMsg));
                    $this->set('NoteMsg',$NoteMsg);
                }

				$this->request->data['User']['last_name'] = $this->request->data['lastname'];
				$this->request->data['User']['mobile'] = $mobile;
//				$this->request->data['User']['area'] = $areaname;
//				$this->request->data['User']['areacode'] = $areacode;
				$this->request->data['User']['telephone'] = $this->request->data['telephone'];
				$this->request->data['User']['block'] = $this->request->data['block'];
				$this->request->data['User']['street'] = $this->request->data['street'];
//				$this->request->data['User']['avenue'] = $this->request->data['avenue'];
				$this->request->data['User']['bilding'] = $this->request->data['bildng'];
				$this->request->data['User']['floor'] = $this->request->data['floor'];
				$this->request->data['User']['aprtmnt'] = $this->request->data['aprtmnt'];
				
				$this->request->data['User']['city'] = $this->request->data['area'];
...
//				$this->request->data['User']['address2'] = $this->request->data['address2'];
				...
				if($usr_datas['User']['email'] !== $this->request->data['setting-email']){
					$emailaddress = $this->request->data['User']['email'] = $this->request->data['setting-email'];
					if(!empty($this->request->data['refferid'])){
						$reff_id['reffid'] = $this->request->data['refferid'];
						$reff_id['first'] = 'first';
				...
				$this->User->save($this->request->data);
				$this->Session->setFlash(__('Successfully Updated'), 'default', array(), 'good');
				$this->redirect('/settings/');
				
			}

			$countrylist = $this->Country->find('all');
			$statelist = $this->Allstate->find('all');
			$arealist = $this->Allarea->find('all');
		...
		function suprcountry(){
			...
			$catsdata = array();
			if($suprsub == 'yes'){
				$catsdata = $this->Allstate->find('all',array('conditions'=>array('country_id'=>$cateid)));
			}else{
				$catsdata2 = $this->Allarea->find('all',array('conditions'=>array('state_id'=>$cateid)));
			}

			if(!empty($catsdata)){
				foreach($catsdata as $cts){
					$cats_arr[] = array('ID'=>$cts['Allstate']['id'],'Name'=>$cts['Allstate']['state']);
				}
				// [ { "ID" :"1", "Name":"Scott"},{ "ID":"2", "Name":"Jon"} ]
				// print_r($cats_arr);
				echo json_encode($cats_arr);
				
			}elseif(!empty($catsdata2)){
				foreach($catsdata2 as $cts){
					$cats_arr[] = array('ID'=>$cts['Allarea']['id'],'Name'=>$cts['Allarea']['area']);
				}
				echo json_encode($cats_arr);
		...
		function shpng_country(){
			...
			if($suprsub == 'yes'){
				...
				$catsdata = $this->State->find('all',array('conditions'=>array('country_id'=>$cateid)));
			}else{
				$catsdata2 = $this->Area->find('all',array('conditions'=>array('state_id'=>$cateid)));
			}

			if(!empty($catsdata)){
				foreach($catsdata as $cts){
					$cats_arr[] = array('ID'=>$cts['State']['id'],'Name'=>$cts['State']['state']);
				}
				// [ { "ID" :"1", "Name":"Scott"},{ "ID":"2", "Name":"Jon"} ]
				// print_r($cats_arr);
				echo json_encode($cats_arr);
				...
				foreach($catsdata2 as $cts){
					$cats_arr[] = array('ID'=>$cts['Area']['id'],'Name'=>$cts['Area']['area']);
				}
				echo json_encode($cats_arr);
        ...
        function country_phonecode(){
			...
			$catsdata = array();
			$catsdata = $this->Country->find('all',array('conditions'=>array('id'=>$cateid)));
//				$country_id = $cntry_id[0]['Country']['id'];

			if(!empty($catsdata)){
				foreach($catsdata as $cts){
					$cats_arr = $cts['Country']['phone_code'];
				}
				echo json_encode($cats_arr);
			}else{
				echo 0;
		...
		function password_change(){
			...
			if(!empty($this->request->data)){
				...
				$this->redirect('/settings/');
					...
					$this->Session->setFlash(__('Existing password is incorrect'));
					$this->redirect($this->referer());
				}
			}
...
//				$countrycode = $this->request->data['cntry'];
//				$countryModel = $this->Country->find('all',array('conditions'=>array('id'=>$countrycode)));
//				$countryname = $countryModel[0]['Country']['country'];
				
				$countrycode = $this->request->data['cntry'];
				$countryModel = $this->Allowedcountries->findByid($countrycode);
				...
				$this->request->data['Tempaddresses']['city'] = $this->request->data['area'];
			...
			$countrylist = $this->Allowedcountries->find('all');
			$this->set('countrylist',$countrylist);
			...
			if($itemfavs <= 0){
				...
				if ($userid != $userdatasall['Item']['user_id']) {
					...
					if ($getuserIdd['User']['id'] != $userid){
						...
						$userddett = $this->Userdevice->find('all',array('conditions'=>array('user_id'=>$getuserIdd['User']['id'])));
		...
		function sellersignup($additemid=NULL){
			...
			if(!empty($this->request->data)){
				...
				if ($status == 1){
					$this->Session->setFlash(__('Merchant Updated'), 'default', array(), 'good');
		...
		$this->Email->to = $emailaddress;
		$this->Email->subject = $setngs[0]['Sitesetting']['site_name']." – Seller Approval Request";
		$this->Email->from = SITE_NAME."<".$setngs[0]['Sitesetting']['noreply_email'].">";
		$this->Email->sendAs = "html";
		$this->Email->template = 'sellerrequest';		
		$this->set('email', $emailaddress);
		$this->set('username',$username);
		$this->set('access_url',SITE_URL."login");
		$this->Email->send(); 
					$this->Session->setFlash(__('Admin Approval Pending'), 'default', array(), 'good');
					$this->redirect('/');
				}
			}else{
				...
				if(empty($shopdetails['Shop']['paypal_id'])){
					$NoteMsg="<b>Note:</b> You need to signup this seller form to add your product for sales in this website. You are signup will be approved by the admin in 24 hours of working time and after successful approval, you can add your products and it's details.";
					$this->set('NoteMsg',$NoteMsg);
					//$this->Session->setFlash("Note: You need to signup this seller form to add your product for sales in this website. You are signup will be approved by the admin in 24 hours of working time and after successful approval, you can add your products and it's details.");
				}else{
			...
			if (count($userDetails) > 0) {
				...
				foreach ($userDetails as $userData) {
					...
					if ($userContent == ''){
						...
						$userContent = "<li><img class='photo' src='".$_SESSION['media_url']."media/avatars/thumb70/".$userimg."'  ><input type='hidden' class = 'nam'  value='".$usernam."' /><span class='username'>".$usernameurl."</span></li>";
					}else {
						$userContent = $userContent."<li><img class='photo' src='".$_SESSION['media_url']."media/avatars/thumb70/".$userimg."'><input type='hidden' class = 'nam'  value='".$usernam."' /><span class='username'>".$usernameurl."</span></li>";
					}
					$k++;
				}
			} else {
				...
				if (!empty($user_profile)) {
					...
					if(!empty($user)){
						...
						if($user_status=='disable'){
							$this->Session->setFlash(__('Your account has been disbled please contact our support'), 'default', array(), 'bad');
							$this->redirect('/login');
						}
			...
			$hybridauth_config = array(
					...
					"providers" => array(
							"Twitter" => array(
									"enabled" => true,
									//"keys" => array("key" => "IGwaIlxkJPiYcqD644p4Zw", "secret" => "nfHk0HW5SgnP4Sd51OuqCuK29p3vTI9NqRnQXs7k")
									"keys" => array("key" => TWITTER_ID, "secret" => TWITTER_SECRET)
							)
							// for another provider refer to hybridauth documentation
					)
			);
		...
		public function twittlogin_save(){
			...
			if(!empty($this->request->data)){
				...
				// echo "<pre>";print_r($nmecounts);die;
				if($username==''){
					$this->Session->setFlash(__('Please enter the Username'));
					$this->redirect($this->referer());
				}
				if($email=='' ){
					$this->Session->setFlash(__('Please enter the Email'));
					$this->redirect($this->referer());
				}
		...
		public function invite_twit_msg(){
			...
			$connection = new TwitterOAuth(TWITTER_ID, TWITTER_SECRET, '301372245-FQLQO7oZI5cbgvTkYVLTbFdJUUsmEQwXKds2iJuR', 'CzG2j2jFuUsVaAfo0gJ7ZZjYTFOmHxniI9JPxXsKg');
		...
		public function invite_friends($provider=null){
			...
			$banner_datas = $this->Banner->find('first',array('conditions'=>array('Banner.banner_type'=>'invitefriends')));
			$this->set('banner_datas',$banner_datas);			
		...
		public function sendinviteemail() {
			...
			foreach ($emailcontacts as $userr){
				...
				$this->Email->from = $setngsemail;
				$this->Email->subject = SITE_NAME." – $firstname invites you to join ".SITE_NAME.".";
				$this->Email->from = SITE_NAME."<".$setngs[0]['Sitesetting']['noreply_email'].">";
				$this->Email->sendAs = "html";
			...
			$this->Session->setFlash(__('Invite sent successfully'));
			$this->redirect('/');
		...
		public function sendinviteemailref() {
			...
			foreach ($email as $key => $toemail){
				...
				$userinviteModel = $this->Userinvite->find('all',array('conditions'=>array('user_id'=>
						$user_id,'invited_email'=>$toemail)));
				if (count($userinviteModel) == 0){
					$this->Userinvite->create();
					$this->request->data['Userinvite']['user_id'] = $user_id;
					$this->request->data['Userinvite']['invited_email'] = $toemail;
					$this->request->data['Userinvite']['status'] = 'Invited';
					$this->request->data['Userinvite']['invited_date'] = time();
					$this->request->data['Userinvite']['cdate'] = time();
					$this->Userinvite->save($this->request->data);
		...
		public function bookmarklet() {
			...
			$this->redirect(SITE_URL.'/bookmark1.php?uploaded='.$image_url);
				...
				$redirect = str_replace("http://", "", SITE_URL);
				$this->redirect(SITE_URL.'/bookmark.php?notloggin=yes&mainurl='.$redirect);
			}
		}
...
$loguserdetails = $this->Log->find('all',array('conditions' =>array('user_id' =>$flwruserid),'order'=>array('id'=>'desc')));
			...
			//$loguserdetails = $this->Log->find('all',array('conditions' =>array('OR' => array(array('user_id' =>$userid),array('type' => 'follow','follow_id' =>$userid))),'order'=>array('id'=>'desc')));
		...
		public function additemusingurl () {
			...
			//echo "$checkUrl[0] <pre>";print_r($checkUrl);die;
			if ($checkUrl[0] != 'http' && $checkUrl[0] != 'https'){
				$url = "http://".$url;
			}
			$checkUrl = '';
			$checkUrl = explode('.',$url);
			$lastIndex = count($checkUrl) - 1;
			//echo "$lastIndex <pre>";print_r($checkUrl);die;
			$extensions = array('JPG', 'TIF', 'PNG', 'GIF', 'JPEG', 'jpg', 'jpeg', 'tif', 'gif', 'png', 'bmp', 'psd');
			if (!in_array($checkUrl[$lastIndex], $extensions)){
		...
		public function searches($searchWord=null) {
			...
			$itemDetails = $this->Item->find('all',array('conditions'=>array('Item.item_title LIKE'=>"%$searchWord%",'Item.status <>'=>"draft")));
		...
		public function nearme() {
			...
			if($setngs[0]['Sitesetting']['affiliate_enb']=='enable'){
				$items_data = $this->Item->find('all',array('conditions'=>array('Shop.shop_latitude BETWEEN ? AND ?' => array($LatS,$LatN) , 'Shop.shop_longitude BETWEEN ? AND ?' => array($LonW,$LonE),'status <>'=>'draft'),'limit'=>'20','order'=>array('Item.id'=>'desc')));
			}else{
				$items_data = $this->Item->find('all',array('conditions'=>array('Shop.shop_latitude BETWEEN ? AND ?' => array($LatS,$LatN) , 'Shop.shop_longitude BETWEEN ? AND ?' => array($LonW,$LonE),'status'=>'publish'),'limit'=>'20','order'=>array('Item.id'=>'desc')));
			}
		...
		function getMorenearme() {
			...
			if ($_GET['loadmoretab'] == 'following') {
				...
				if($setngs[0]['Sitesetting']['affiliate_enb']=='enable'){
					...
					$items_data = $this->Item->find('all',array('conditions'=>array('status <>'=>'draft','Item.user_id'=>$followingId,'Shop.shop_latitude BETWEEN ? AND ?' => array($LatS,$LatN) , 'Shop.shop_longitude BETWEEN ? AND ?' => array($LonW,$LonE)),'limit'=>$offset,'offset'=>$startIndex,'order'=>array('Item.id'=>'desc')));
				}else{
					$items_data = $this->Item->find('all',array('conditions'=>array('status'=>'publish','Item.user_id'=>$followingId,'Shop.shop_latitude BETWEEN ? AND ?' => array($LatS,$LatN) , 'Shop.shop_longitude BETWEEN ? AND ?' => array($LonW,$LonE)),'limit'=>$offset,'offset'=>$startIndex,'order'=>array('Item.id'=>'desc')));
				}
			...
			}elseif (isset($_GET['loadmoretab']) && $_GET['loadmoretab'] == 'featured'){
				if($setngs[0]['Sitesetting']['affiliate_enb']=='enable'){
						$items_data = $this->Item->find('all',array('conditions'=>array('Item.status <>'=>'draft','Item.featured'=>'1','Shop.shop_latitude BETWEEN ? AND ?' => array($LatS,$LatN) , 'Shop.shop_longitude BETWEEN ? AND ?' => array($LonW,$LonE)),'limit'=>$offset,'offset'=>$startIndex,'order'=>array('Item.id'=>'desc')));
				}else{
						$items_data = $this->Item->find('all',array('conditions'=>array('Item.status'=>'publish','Item.featured'=>'1','Shop.shop_latitude BETWEEN ? AND ?' => array($LatS,$LatN) , 'Shop.shop_longitude BETWEEN ? AND ?' => array($LonW,$LonE)),'limit'=>$offset,'offset'=>$startIndex,'order'=>array('Item.id'=>'desc')));
				}
				...
				if($setngs[0]['Sitesetting']['affiliate_enb']=='enable'){
					//$items_data = $this->Item->find('all',array('conditions'=>array('status <>'=>'','Item.user_id'=>$followingId),'limit'=>$offset,'offset'=>$startIndex,'order'=>array('Item.id'=>'desc')));
					$items_data = $this->Item->find('all',array('conditions'=>array('Item.status <>'=>'draft','Shop.shop_latitude BETWEEN ? AND ?' => array($LatS,$LatN) , 'Shop.shop_longitude BETWEEN ? AND ?' => array($LonW,$LonE)),'limit'=>$offset,'offset'=>$startIndex,'order'=>array('Item.id'=>'desc')));
				}else{
					$items_data = $this->Item->find('all',array('conditions'=>array('Item.status'=>'publish','Shop.shop_latitude BETWEEN ? AND ?' => array($LatS,$LatN) , 'Shop.shop_longitude BETWEEN ? AND ?' => array($LonW,$LonE)),'limit'=>$offset,'offset'=>$startIndex,'order'=>array('Item.id'=>'desc')));
				}
		...
		function sellerpost(){
			...
			if(!empty($this->request->data)){
				...
				$this->loadModel('Userdevice');
				$this->loadModel('Follower');
				$flwrscnt = $this->Follower->flwrscnt($logusrid);
				foreach($flwrscnt as $flwww){
					$useriddd = $flwww['Follower']['follow_user_id'];
		...
		function changeuserimgstatuss ($fId,$status) {
			...
			$this->loadModel('User');
			$this->loadModel('Item');
			$selfname=$loguser[0]['User']['first_name'];
			$selfurl=$loguser[0]['User']['username_url'];
			$usid = $this->Fashionuser->findByid($fId);
			$ueruploid=$usid['Fashionuser']['user_id'];
			$itemdet=$usid['Fashionuser']['itemId'];
			
			$usidemail = $this->User->findByid($ueruploid);
			$ueruploidemail=$usidemail['User']['email'];
			$ueruploidfname=$usidemail['User']['first_name'];
			
			
			$itemdeta = $this->Item->findByid($itemdet);
			$ueruploidemailss=$itemdeta['Item']['user_id'];
			$item_title_url=$itemdeta['Item']['item_title_url'];
			$appem = $this->User->findByid($ueruploid);
			$approvalemail=$appem['User']['email'];
			
			$this->set('ueruploidfname',$ueruploidfname);
			$this->set('fId',$fId);
			$this->set('itemdet',$itemdet);
			$this->set('item_title_url',$item_title_url);
			$this->set('ueruploidfname',$ueruploidfname);
			...
			if ($status == 'Yes') {
				...
				$result = "<button class='btn btn-success' onclick='changeUserImgStatus(".$fId.",\"No\");' style='display:inline-table;'>Show<img id='loadd".$fId."' src='".SITE_URL."images/loading.gif' style='width: 13px; position: absolute; margin-top: 4px; margin-left: 6px; display: none;'></button>";
				/* $this->Email->smtpOptions = array(
						'port' => '465',
						'timeout' => '30',
						'host' => 'ssl://smtp.gmail.com',
				...
				$this->Email->delivery = 'smtp';
				$this->Email->to = $ueruploidemail;
				$this->Email->subject = "Your fashion upload was approved by seller.";
				$this->Email->from = SITE_NAME."<".$setngs[0]['Sitesetting']['noreply_email'].">";
				$this->Email->sendAs = "html";
				$this->Email->template = 'uploadfashionapproval';
				...
				$result = "<button class='btn btn-warning' onclick='changeUserImgStatus(".$fId.",\"Yes\");' style='display:inline-table;'>Hide<img id='loadd".$fId."' src='".SITE_URL."images/loading.gif' style='width: 13px; position: absolute; margin-top: 4px; margin-left: 6px; display: none;'></button>";
				...
				$this->Email->to = $ueruploidemail;
				$this->Email->subject =  $setngs[0]['Sitesetting']['site_name']." – Your fashion upload was approved by seller";
				$this->Email->from = SITE_NAME."<".$setngs[0]['Sitesetting']['noreply_email'].">";
				$this->Email->sendAs = "html";
				$this->Email->template = 'uploadfashionapproval';
				
				$this->set('selfname',$selfname);
