	...
	class PaypalsController  extends AppController{
		...
		function paycart(){
			...
			if(!empty($listingid) && $itemUserId != $userid){
				...
				$cntryExist = $this->User->find('all',array('conditions'=>array('User.id'=>$userid)));
				$cntryExist_code = $cntryExist[0]['User']['countrycode'];
				
				$areaid = $cntryExist[0]['User']['city'];
				$areaModel = $this->Allarea->findById($areaid);
				$areaname = $areaModel['Allarea']['area'];

				$stateid = $cntryExist[0]['User']['state'];
                ...
                $userDetails = $this->Allowedcountries->query("SELECT * FROM ".$prefix."allowed_countries Where code = '{$cntryExist_code}'");
				...
				$this->request->data['Cart']['created_at'] = date("Y-m-d H:i:s");
					if(!empty($userDetails) && !empty($userSDetails) && !empty($userADetails)){
				...
				$this->Session->setFlash('You cannot purchase your own Product.');
				$this->redirect('/');
			}else{
		...
		function pay(){
            ...
            $del_area_splr = array();
            foreach($itm_datas as $itm) {
                $del_area_user = $this->Deliverycharge->find('all',array('conditions'=>array('Deliverycharge.name'=>$ship['Tempaddresses']['city'])));
                $area_chrg_datas = $this->Deliverycharge->find('all',array('conditions'=>array('Deliverycharge.name'=>$itm['User']['city'])));
                $area_chrg_dval = $area_chrg_datas[0]['Deliverycharge'];
                array_push($del_area_splr, $area_chrg_dval);
            }
            $this->set('del_area_user',$del_area_user);
            $this->set('del_area_splr',$del_area_splr);

/*            if($total_itms == 1){
                if($itm_datas[0]['Item']['delivery_type'] == 'regular'){
                    $charg = $del_area_splr[0]['regulr_chrge'];
                }else{
                    $charg2 = 0.0;
                    $cartdats = $this->Cart->find('all',array('conditions'=>array('user_id'=>$userid,'item_id'=>$itm['Item']['id'], 'payment_status'=>'progress')));
                    $this->request->data['Cart']['shipping_charg'] = $charg2;
                    $this->request->data['Cart']['shipping_status'] = 'disable';
                ...
                $area_data2 = array();
              	foreach($itm_datas as $itm) {
			        if($itm['Item']['delivery_type'] == 'regular'){
                        ...
                        $area2 = $itm['Item'];
                        array_push($area_data2, $area2);
                    }
                }
                if(!empty($area_data)){
                    if($area_data[0] == $area_data[1]){
                        $charg = $del_area_splr[0]['regulr_chrge'];
                    }elseif($del_area_splr[0]['regulr_chrge'] > $del_area_splr[1]['regulr_chrge']){
                        $charg = $del_area_splr[0]['regulr_chrge'];
                    }else{
                        $charg = $del_area_splr[1]['regulr_chrge'];
                    }
                }else{
                    $charg2 = 0.0;
                    foreach($area_data2 as $del_item_id){
                        $cartdats = $this->Cart->find('all',array('conditions'=>array('user_id'=>$userid,'item_id'=>$del_item_id, 'payment_status'=>'progress')));
                        $this->request->data['Cart']['shipping_charg'] = $charg2;
                        $this->request->data['Cart']['shipping_status'] = 'disable';
                        $this->request->data['Cart']['id'] = $cartdats[0]['Cart']['id'];
                        $this->Cart->save($this->request->data);
            ...
            if($total_itms > 0){
                ...
                $area_data2 = array();
              	foreach($itm_datas as $itm) {
                    if($itm['Item']['delivery_type'] == 'regular'){
                        $area = $itm['User']['city'];
                        $area_id = $this->Area->find('all',array('conditions'=>array('Area.id'=> $usershipping[0]['Tempaddresses']['city'])));
                        $area_val = $area_id[0]['Area']['area'];
                        $del_area_id = $this->Deliverycharge->find('all',array('conditions'=>array('Deliverycharge.name'=> $area_val)));
                        $del_val = $del_area_id[0]['Deliverycharge']['regulr_chrge'];
                        array_push($area_data, $del_val);
                    }else{
                        $area2 = $itm['Item']['id'];
                        array_push($area_data2, $area2);
                    }
                ...
                if(!empty($area_data)){
                    $max_val = '0.0';
                  	foreach($area_data as $area_data_val) {
                        if($max_val < $area_data_val){
                            $max_val = $area_data_val;
                        }
                    }
                    $add = 0;
                    $total_area_itms = count($area_data);
                    for($i=3;$i<=$total_area_itms;$i++){
                        $add++;
                    }
                    $charg = $max_val + $add;
                }
                if(!empty($area_data2)){
                    ...
                    foreach($area_data2 as $del_item_id){
                        $cartdats = $this->Cart->find('all',array('conditions'=>array('user_id'=>$userid,'item_id'=>$del_item_id, 'payment_status'=>'progress')));
                        if($cartdats[0]['Cart']['shipping_charg'] == NULL){
                            $this->request->data['Cart']['shipping_charg'] = $charg2;
                            $this->request->data['Cart']['shipping_status'] = 'disable';
                            $this->request->data['Cart']['id'] = $cartdats[0]['Cart']['id'];
                            $this->Cart->save($this->request->data);
            ...
            $car_spl_ids = $this->Cart->find('all');
        //    var_dump($charg);            
      //      var_dump($charg2);
	//		var_dump($area_data);
//			var_dump($area_data2);
			$this->set('charg',$charg);
			$this->set('charg2',$charg2);
			$this->set('car_spl_ids',$car_spl_ids);
	...
	function checkout () {
			...
			if ($adaptive == "paypalnormal" || $normal == 1) {
				...
				$createPacket = array(
						...
						"returnUrl" => SITE_URL.'payment-successful',
						"cancelUrl" => SITE_URL.'payment-cancelled',
						"ipnNotificationUrl" => SITE_URL.'paypal/adaptiveipnprocess/',//'http://dev.hitasoft.com/new/success.php',
		...
		function ipnprocess () {
			...
			$result = curl_exec($ch);
			curl_close($ch);
			//$previousTrack = $this->Invoices->find('all',array('conditions'=>array('paypaltransactionid'=>$keyarray['txn_id'])));
			...
			if ($result == 'VERIFIED' && $keyarray['payment_status'] == 'Completed') {
				...
				$forexrateModel = $this->Forexrates->find('first',array('conditions'=>array('currency_code' => $currencyCode)));
				...
				foreach ($itemUsers as $itemUser) {
					...
					$this->request->data['Orders']['status'] = "Pending";
					...
					//$this->request->data['Invoices']['paypaltransactionid'] = $keyarray['txn_id'];
					$this->Invoices->save($this->request->data);
					$invoiceId = $this->Invoices->getLastInsertID();
					...
					$userModelemail = $this->User->findById($user_id);
					$usershipping_addr = $this->Shippingaddresses->findByShippingid($shippingId);
					if($setngs[0]['Sitesetting']['gmail_smtp'] == 'enable'){
						$this->Email->smtpOptions = array(
							'port' => $setngs[0]['Sitesetting']['smtp_port'],
							'timeout' => '30',
					...
					$this->Email->to = $userModelemail['User']['email'];
					//$this->Email->to = 'saravananm@hitasoft.com';
					$this->Email->subject = SITE_NAME." – You have order(s) #$orderId";
					$this->Email->from = SITE_NAME."<".$setngs[0]['Sitesetting']['noreply_email'].">";
					$this->Email->sendAs = "html";
					$this->Email->template = 'ipnmail_supp';
					$this->set('custom',$userModelemail['User']['first_name']);
					...
					$this->set('buyername',$userModel['User']['username']);
					$this->set('buyernameurl',$userModel['User']['username_url']);
				...
				$this->Email->to = $custom[0];
				$this->Email->subject = SITE_NAME." – Confirmation of your order number(s) $ordersId";
				$this->Email->from = SITE_NAME."<".$setngs[0]['Sitesetting']['noreply_email'].">";
				$this->Email->sendAs = "html";
				$this->Email->template = 'ipnmail_cust';
				$this->set('custom',$usernameforcust);
...
//			var_dump($area_data2);
		...
		public function checkcouponcode ($id = NULL) {
			...
			if(!empty($getcouponval)){
				...
				$coupon_use = $getcouponval['Coupon']['one_time_use'];
			...
			if($coupon_use == "yes")
			{
				$log_coupon_datas = $this->Logcoupon->find('count',array('conditions'=>array('Logcoupon.coupon_id'=>$couponid,'Logcoupon.user_id'=>$userId)));
				if($log_coupon_datas>0)
				{
					echo "4";die;
				}
		...
		function ggipn () {
			...
			if ($result == 'VERIFIED' && $keyarray['payment_status'] == 'Completed' ) {
				...
				$itemcost = $ggitemDetails['Groupgiftuserdetail']['total_amt'];
				$itemsize = $ggitemDetails['Groupgiftuserdetail']['itemsize'];
				$itemquantity = $ggitemDetails['Groupgiftuserdetail']['itemquantity'];
				...
				$this->request->data['Shippingaddresses']['userid'] = $ggcreateuserId;
				$this->request->data['Shippingaddresses']['name'] = $name;
				$this->request->data['Shippingaddresses']['nickname'] = time();
				$this->request->data['Shippingaddresses']['country'] = $countryName;
				$this->request->data['Shippingaddresses']['state'] = $state;
				$this->request->data['Shippingaddresses']['address1'] = $address1;
				$this->request->data['Shippingaddresses']['address2'] = $address2;
				$this->request->data['Shippingaddresses']['city'] = $city;
				$this->request->data['Shippingaddresses']['zipcode'] = $zipcode;
				$this->request->data['Shippingaddresses']['phone'] = $telephone;
				$this->request->data['Shippingaddresses']['countrycode'] = $country;
				$this->Shippingaddresses->save($this->request->data);
				$shippingid =  $this->Shippingaddresses->getLastInsertID();
				...
				if($balance_amt <= 0 ){
					...
					$this->request->data['Orders']['totalcost'] = $itemcost;
					$this->request->data['Orders']['orderdate'] = time();
					$this->request->data['Orders']['shippingaddress'] = $shippingid;
					$this->request->data['Orders']['coupon_id'] = '0';
					$this->request->data['Orders']['discount_amount'] = '0';
					$this->request->data['Orders']['totalCostshipp'] = $shipcost;					
					$this->Orders->save($this->request->data);
					...
					$orderId = $this->Orders->getLastInsertID();
						...
						$this->request->data['Order_items']['itemprice'] = $itemcost - $shipcost;
						$this->request->data['Order_items']['itemquantity'] = $itemquantity;
						$this->request->data['Order_items']['itemunitprice'] = ($itemcost - $shipcost) / $itemquantity;
						$this->request->data['Order_items']['shippingprice'] = $shipcost;
						$this->request->data['Order_items']['item_size'] = $itemsize;
						$this->Order_items->save($this->request->data);
						...
						if(!empty($itemopt)){
							...
							foreach($explodedcoma as $k=>$exdecoma){
								...
								$explodedequal = explode('=',$exdecoma);
								if($explodedequal[0] == $itemsize){
									//echo "   ".$llll;
									$reduced = $explodedequal[1] - $itemquantity;
									$getVall = $itemsize."=".$reduced;
									//echo "<pre>";print_r($sss);
						...
						$this->request->data['Item']['quantity'] = $quantityItem - $itemquantity;
						$this->request->data['Item']['size_options'] = $final;
						$this->Item->save($this->request->data);
